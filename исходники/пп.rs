...
use синтаксис::{Процедура, Модуль, Утверждение, Выражение};
use std::io;
use std::io::Write;
...
#[derive(Debug)]
pub enum Инструкция {
    /// Протолкнуть значение на стек аргументов
    Протолкнуть(usize),
    ПечатьСтроки,
    Возврат
}
...
impl Программа {
    fn скомпилировать_утвержление(&mut self, утверждение: &Утверждение) -> Option<()> {
        match утверждение {
            Утверждение::Присваивание{..} => todo!(),
                        ...
                        for арг in аргументы {
                            match арг {
                                Выражение::Строка(строка) => {
                                    ...
                                    self.код.push(Инструкция::Протолкнуть(длинна));
                                    self.код.push(Инструкция::Протолкнуть(указатель));
                                    self.код.push(Инструкция::ПечатьСтроки);
                                }
                                Выражение::Число(_) => todo!(),
                            }
                        }
                        Some(())
                    },
    ...
    fn скомпилировать_процедуру(&mut self, процедура: &Процедура) -> Option<usize> {
    ...
    pub fn скомпилировать_модуль(&mut self, модуль: &Модуль) -> Option<()> {
        assert!(модуль.переменные.len() == 0, "СДЕЛАТЬ: компиляция глобальных переменных");
    ...
    pub fn интерпретировать(&self, процедура_точки_входа: &str) -> Option<()> {
        ...
        loop {
            ...
            match инструкция {
                Инструкция::Протолкнуть(значение) => {
                    стек.push(*значение);
                    индекс_инструкции += 1;
                }
