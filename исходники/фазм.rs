...
fn сгенерировать_инструкции(файл: &mut impl Write, пп: &ПП, точка_входа_программы: usize) -> Результат<()> {
    ...
    for (индекс, инструкция) in пп.код.iter().enumerate() {
        ...
        match &инструкция.вид {
            ...
            ВидИнструкции::ВыделитьНаВторомСтеке(_размер) => {
                сделать!(&инструкция.лок, "Генерация инструкции {вид_инструкции:?} для фазм", вид_инструкции = инструкция.вид);
                return Err(());
            }
            ВидИнструкции::ОсвободитьСоВторогоСтека(_размер) => {
                сделать!(&инструкция.лок, "Генерация инструкции {вид_инструкции:?} для фазм", вид_инструкции = инструкция.вид);
                return Err(());
            }
