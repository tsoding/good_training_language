...
impl<'a> Лексер<'a> {
    ...
    fn следующая_лексема(&mut self) -> Option<Лексема> {
        ...
        if сим.is_alphabetic() {
            ...
            for &(ключ, вид) in КЛЮЧЕВЫЕ_СЛОВА.iter() {
                if ключ == текст {
                    return Some(Лексема {вид, текст, лок})
                }
            }
            return Some(Лексема {
    ...
    pub fn вытащить_лексему(&mut self) -> Option<Лексема> {
    ...
    pub fn вытащить_лексему_вида(&mut self, ожидаемые_виды: &[ВидЛексемы]) -> Option<Лексема> {
