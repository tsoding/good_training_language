...
pub const ПРИСТАВКИ_ПРЕПИНАНИЙ: &[(&[char], ВидЛексемы)] = &[
    ...
    (&[','], ВидЛексемы::Запятая),
    (&['+', '!', '='], ВидЛексемы::МеньшеЛи),
    (&['+', '!'], ВидЛексемы::МеньшеРавноЛи),
    (&['+', '?', '='], ВидЛексемы::БольшеРавноЛи),
    (&['+', '?'], ВидЛексемы::БольшеЛи),
    (&['+'], ВидЛексемы::Плюс),
    (&['-', '!', '='], ВидЛексемы::БольшеЛи),
    (&['-', '!'], ВидЛексемы::БольшеРавноЛи),
...
impl ВидЛексемы {
    pub fn сущ(&self) -> Сущ {
        match self {
            ...
            ВидЛексемы::РавноЛи            => Сущ{текст: "«равно ли»",                 род: Род::Сред},
            ВидЛексемы::МеньшеЛи           => Сущ{текст: "«меньше ли»",                род: Род::Сред},
            ВидЛексемы::БольшеЛи           => Сущ{текст: "«больше ли»",                род: Род::Сред},
            ВидЛексемы::МеньшеРавноЛи      => Сущ{текст: "«меньше либо равно ли»",     род: Род::Сред},
            ВидЛексемы::БольшеРавноЛи      => Сущ{текст: "«больше либо равно ли»",     род: Род::Сред},
            ВидЛексемы::Не                 => Сущ{текст: "«не»",                       род: Род::Сред},
            ВидЛексемы::НеРавноЛи          => Сущ{текст: "«не равно ли»",              род: Род::Сред},
...
impl<'a> Лексер<'a> {
    ...
    fn подбрить_многострочные_комментарии(&mut self) {
        ...
        while вложенность > 0 && self.текущий_символ().is_some() {
    ...
    fn следующая_лексема(&mut self) -> Результат<Лексема> {
        ...
        if сим == '«' {
            ...
            loop {
                if let Some(сим) = self.текущий_символ().cloned() {
                    match сим {
                        '«' => {
                            self.отрезать_символ();
                            вложенность += 1;
                        ...
                        '\\' => {
                            ...
                            match self.текущий_символ().cloned() {
                                ...
                                Some('«') | Some('»') => {
                                    ...
                                    диагностика!(&лок, "ОШИБКА", "Экранировать «ёлочеки» внутри \"лапок\" не нужно!");
                                    return Err(());
                                }
                                Some(сим) => {
                                    let лок = Лок {
