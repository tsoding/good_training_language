...
pub const ПРИСТАВКИ_ПРЕПИНАНИЙ: &[(&[char], ВидЛексемы)] = &[
    ...
    // СДЕЛАТЬ: Мне кажется, стоит вместо процента использовать что-то типа ключевого слова «ост» (от «остаток»). Возможно так стоит ввести слово «мод» специально для взятие значения числа по модулю.
    (&['%'], ВидЛексемы::Процент),
...
impl ВидЛексемы {
    pub fn сущ(&self) -> Сущ {
        match self {
            ...
            ВидЛексемы::НатЧисло           => Сущ{текст: "натуральное число",          род: Род::Сред},
