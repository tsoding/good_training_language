...
pub fn сгенерировать(путь_к_файлу: &Path, пп: &ПП, точка_входа_программы: usize) -> Результат<()> {
    ...
    for инструкция in &пп.код {
        ...
        match инструкция {
            ...
            &Инструкция::ПротолкнутьУказатель(смещение) => {
                код.extend([0x68]); // push
            ...
            &Инструкция::Вытолкнуть(количество) => {
            ...
            &Инструкция::ПрочитатьКадр(смещение) => {
                код.extend([0x48, 0x8B, 0x85]);                   // mov rax,
                код.extend((-(смещение as i32)-1).to_le_bytes()); // [rbp-смещение-1]
                код.extend([0x50]);                               // push rax
            }
            &Инструкция::ЗаписатьКадр(_смещение) => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЗаписатьКадр");
            }
            &Инструкция::ВызватьПроцедуру(индекс_инструкции_пп_цели) => {
                код.extend([0xE8]); // call
            ...
            Инструкция::ЦелБольше => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелБольше");
            }
            ...
            Инструкция::ЦелВычитание => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелВычитание");
            }
            Инструкция::ЦелУмножение => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелУмножение");
            }
            Инструкция::ЦелОстаток => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелОстаток");
            }
            Инструкция::ЦелДеление => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелДеление");
            }
            Инструкция::ЦелРавно => {
                todo!("СДЕЛАТЬ: кодогенерация инструкции ЦелРавно");
            }
