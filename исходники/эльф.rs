...
pub fn сгенерировать(путь_к_файлу: &Path, пп: &ПП, точка_входа_программы: usize) -> Результат<()> {
    ...
    let mut заплатки_указателей_на_иниц_данные: Vec<ЗаплаткаУказателяНаДанные> = Vec::new();
    let mut заплатки_указателей_на_неиниц_данные: Vec<ЗаплаткаУказателяНаДанные> = Vec::new();
    ...
    for инструкция in &пп.код {
        ...
        match инструкция {
            ...
            &Инструкция::ПротолкнутьИницУказатель(смещение) => {
                код.extend([0x68]); // push
                ...
                заплатки_указателей_на_иниц_данные.push(ЗаплаткаУказателяНаДанные {
                    смещение,
                    адрес_операнда_указателя,
                });
            }
            &Инструкция::ПротолкнутьНеиницУказатель(смещение) => {
                код.extend([0x68]); // push
                let адрес_операнда_указателя = код.len();
                код.extend([0x00, 0x00, 0x00, 0x00]); // Заполняем операнд нулями, т.к. реальный указатель будет известен позже.
                заплатки_указателей_на_неиниц_данные.push(ЗаплаткаУказателяНаДанные {
                    смещение,
                    адрес_операнда_указателя,
                });
            }
    ...
    for ЗаплаткаУказателяНаДанные {
        адрес_операнда_указателя,
        смещение,
    } in заплатки_указателей_на_иниц_данные {
    ...
    for ЗаплаткаУказателяНаДанные {
        адрес_операнда_указателя,
        смещение,
    } in заплатки_указателей_на_неиниц_данные {
        let операнд = &mut код[адрес_операнда_указателя..адрес_операнда_указателя+4];
        let указатель_на_данные = начало_данных as i32 + пп.иниц_данные.len() as i32 + смещение as i32;
        операнд.copy_from_slice(&указатель_на_данные.to_le_bytes());
    }
