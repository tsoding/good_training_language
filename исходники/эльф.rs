...
pub fn сгенерировать_объектный_файл(путь_к_файлу: &Path) -> Результат<()> {
    let строки = &[".symtab", ".strtab", ".rela.text", ".text", ".data", "_start", "printf", "_exit"];
    let mut байты_строк: Vec<u8> = Vec::new();
    let mut смещения_строк: HashMap<&str, usize> = HashMap::new();
    байты_строк.extend([0]);
    for строка in строки {
        смещения_строк.insert(строка, байты_строк.len());
        байты_строк.extend(строка.as_bytes());
        байты_строк.extend([0]);
    }
    let размер_секции_строк: Эльф64ХСлово = байты_строк.len() as Эльф64ХСлово;
    выравнять_байты(&mut байты_строк, 8);
    let смещение_секции_строк: Эльф64Смещ = ЭЛЬФ64_РАЗМЕР_ЭЛЬФ_ЗАГОЛОВКА as Эльф64Смещ;

    let mut байты_секции: Vec<u8> = Vec::new();
    байты_секции.extend((*смещения_строк.get(".strtab").unwrap() as Эльф64Слово).to_le_bytes()); // sh_name
    байты_секции.extend((СекцияТип::ТаблицаСтрок as Эльф64Слово).to_le_bytes()); // sh_type
    байты_секции.extend((0 as Эльф64ХСлово).to_le_bytes()); // sh_flags
    байты_секции.extend((0 as Эльф64Адрес).to_le_bytes()); // sh_addr
    байты_секции.extend(смещение_секции_строк.to_le_bytes()); // sh_offset
    байты_секции.extend(размер_секции_строк.to_le_bytes()); // sh_size
    байты_секции.extend((0 as Эльф64Слово).to_le_bytes()); // sh_link
    байты_секции.extend((0 as Эльф64Слово).to_le_bytes()); // sh_info
    байты_секции.extend((1 as Эльф64ХСлово).to_le_bytes()); // sh_addralign
    байты_секции.extend((0 as Эльф64ХСлово).to_le_bytes()); // sh_entsize
    ...
    байты.extend(((ЭЛЬФ64_РАЗМЕР_ЭЛЬФ_ЗАГОЛОВКА + байты_строк.len()) as Эльф64Смещ).to_le_bytes()); // e_shoff
    байты.extend((0 as Эльф64Слово).to_le_bytes()); // e_flags
    байты.extend((ЭЛЬФ64_РАЗМЕР_ЭЛЬФ_ЗАГОЛОВКА as Эльф64Пол).to_le_bytes()); // e_ehsize
    ...
    байты.extend((ЭЛЬФ64_РАЗМЕР_СЕКЦ_ЗАГОЛОВКА as Эльф64Пол).to_le_bytes()); // e_shentsize
    байты.extend((2 as Эльф64Пол).to_le_bytes()); // e_shnum
    байты.extend((1 as Эльф64Пол).to_le_bytes()); // e_shstrndx
    байты.extend(байты_строк);
    байты.resize(байты.len() + ЭЛЬФ64_РАЗМЕР_СЕКЦ_ЗАГОЛОВКА, 0); // Пустая секция
    байты.extend(байты_секции);
