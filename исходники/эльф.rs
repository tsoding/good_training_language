...
pub fn сгенерировать_исполняемый(путь_к_файлу: &Path, пп: &ПП, точка_входа_программы: usize) -> Результат<()> {
    let mut код = vec![];
    ...
    for инструкция in &пп.код {
        ...
        match &инструкция.вид {
            ...
            ВидИнструкции::СисВызов {..} => {
                сделать!(&инструкция.лок, "Кодогенерации инструкции СисВызов");
                return Err(())
            }
...
pub fn сгенерировать_объектный(путь_к_файлу: &Path) -> Результат<()> {
    let mut байты: Vec<u8> = Vec::new();
    байты.extend(ЭЛЬФ64_МПБ_СИС5_ИДЕНТ); // e_ident
