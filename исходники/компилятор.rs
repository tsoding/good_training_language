...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, процедура: &СкомпПроцедура, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(параметр) = процедура.параметры.iter().find(|параметр| параметр.имя.текст == лексема.текст) {
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            match вид {
                ВидБинопа::Меньше => {
                    let левый_тип = скомпилировать_выражение(пп, имена, процедура, &левое, временный_размер)?;
                    let правый_тип = скомпилировать_выражение(пп, имена, процедура, &правое, временный_размер)?;
                ...
                ВидБинопа::Как => {
                    ...
                    let тип_значения = скомпилировать_выражение(пп, имена, процедура, &значение, временный_размер)?;
        ...
        Выражение::УнарныйМинус {ключ, выражение} => {
            let тип = скомпилировать_выражение(пп, имена, процедура, &выражение, временный_размер)?;
            match тип {
                Тип::Вещ32 => {
            ...
            match имя.текст.as_str() {
                ...
                "ввод" => {
                    ...
                    if let Выражение::Идент(имя) = массив {
                        if let Some(_параметр) = процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                ...
                                let тип_размера_среза = скомпилировать_выражение(пп, имена, процедура, &размер_среза, временный_размер)?;
                                проверить_типы(&размер_среза.лок(), &Тип::Цел64, &тип_размера_среза)?;

                                пп.заплатки_неиниц_указателей.push(пп.код.len());
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьУказатель(переменная.адрес), лок: имя.лок.clone() });

                                let тип_начала = скомпилировать_выражение(пп, имена, процедура, &начало_среза, временный_размер)?;
                ...
                "сисвызов" => {
                    ...
                    for аргумент in аргументы {
                        let тип_аргумента = скомпилировать_выражение(пп, имена, процедура, аргумент, временный_размер)?;
                ...
                "адрес" => {
                    ...
                    let тип_среза = скомпилировать_выражение(пп, имена, процедура, &срез, временный_размер)?;
                    match тип_среза {
                        Тип::Срез{..} | Тип::Строка => {
                    ...
                    if let Some(переменная) = имена.переменные.get(&имя.текст) {
                        match &переменная.тип {
                            Тип::Массив{тип_элемента, ..} => {
                                ...
                                let тип_индекса = скомпилировать_выражение(пп, имена, процедура, &индекс, временный_размер)?;
    ...
    match утверждение {
        Утверждение::Присваивание{ключ, имя, значение} => {
            if let Some(параметр) = процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                ...
                let тип_значения = скомпилировать_выражение(пп, имена, процедура, &значение, &mut временный_размер)?;
            ...
            if let Some(переменная) = имена.переменные.get(имя.текст.as_str()) {
                let mut временный_размер = 0;
                let тип = скомпилировать_выражение(пп, имена, процедура, &значение, &mut временный_размер)?;
        ...
        Утверждение::ПрисваиваниеМассива{ключ, имя, индекс, значение} => {
            ...
            let тип_индекса = скомпилировать_выражение(пп, имена, процедура, индекс, &mut временный_размер)?;
            проверить_типы(&индекс.лок(), &Тип::Цел64, &тип_индекса)?;
            пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(тип_элемента.размер()), лок: имя.лок.clone() });
            пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: имя.лок.clone() });
            пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: имя.лок.clone() });
            ...
            match имя.текст.as_str() {
                ...
                "печать" => {
                    ...
                    for арг in аргументы {
                        let тип = скомпилировать_выражение(пп, имена, процедура, &арг, &mut временный_размер)?;
                        match тип {
                            Тип::Строка => {
                ...
                _ => {
                    if let Some(вызываемая_процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        for (параметр, аргумент) in вызываемая_процедура.параметры.iter().zip(аргументы.iter()).rev() {
                            let тип = скомпилировать_выражение(пп, имена, процедура, аргумент, &mut временный_размер)?;
                            проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                            match параметр.тип {
                                Тип::Цел64 | Тип::Лог => {}
                                _ => {
                        ...
                        вызвать_процедуру(имя.лок.clone(), вызываемая_процедура, &mut пп.код, &mut временный_размер)?;
                        if временный_размер > 0 {
                            пп.код.push(Инструкция{ вид: ВидИнструкции::ОсвободитьСоВторогоСтека(временный_размер as u64), лок: имя.лок.clone() });
                        }
                        Ok(())
        ...
        Утверждение::Если{ключ, условие, тело, иначе} => {
            let mut временный_размер = 0;
            let тип = скомпилировать_выражение(пп, имена, процедура, &условие, &mut временный_размер)?;
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                if let Some(результат) = &процедура.результат {
                    let mut временный_размер = 0;
                    let тип = скомпилировать_выражение(пп, имена, процедура, &выражение, &mut временный_размер)?;
