...
#[derive(Default)]
pub struct ЛокальныеПеременные {
    pub стек_областей: Vec<ОбластьВидимости>,
}

impl ЛокальныеПеременные {
    fn найти_по_имени(&self, имя: &str) -> Option<&СкомпПеременная> {
...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &ЛокальныеПеременные, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(переменная) = локальные_переменные.найти_по_имени(&лексема.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::Кадр(переменная.смещение), лок: лексема.лок.clone() });
            ...
            if let Some(параметр) = текущая_процедура.параметры.iter().find(|параметр| параметр.имя.текст == лексема.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::Кадр(параметр.смещение), лок: лексема.лок.clone() });
            ...
            if let Some(переменная) = имена.переменные.get(&лексема.текст) {
                ...
                пп.код.push(Инструкция{ вид: ВидИнструкции::ГлобальныеДанные(переменная.смещение), лок: лексема.лок.clone() });
                if переменная.тип.примитивный() {
                    прочитать_примитивный_тип(&лексема.лок, &mut пп.код, переменная.тип.размер(&имена.структуры))?;
                }
                return Ok(переменная.тип.clone());
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            match вид {
                ВидБинопа::Поле => {
                    let левый_тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &левое, временный_размер)?;
                    match &**правое {
                ...
                ВидБинопа::Меньше => {
                    let левый_тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &левое, временный_размер)?;
                    let правый_тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &правое, временный_размер)?;
                ...
                ВидБинопа::Как => {
                    let значение = левое;
                    let тип = правое;
                    let тип_цели = скомпилировать_тип(тип, имена)?;
                    let тип_значения = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &значение, временный_размер)?;
        ...
        Выражение::УнарныйМинус {ключ, выражение} => {
            let тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &выражение, временный_размер)?;
            match тип {
                Тип::Вещ32 => {
            ...
            match имя.текст.as_str() {
                ...
                "ввод" => {
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, имена, текущая_процедура, локальные_переменные) {
                            if let Тип::Массив{размер, тип_элемента} = &переменная.тип {
                                ...
                                match локация {
                                    Локация::Глобальная => {
                                        пп.заплатки_неиниц_указателей.push(пп.код.len());
                                        пп.код.push(Инструкция{ вид: ВидИнструкции::ГлобальныеДанные(переменная.смещение), лок: имя.лок.clone() });
                                    }
                                    Локация::Стековая => {
                                        пп.код.push(Инструкция{ вид: ВидИнструкции::Кадр(переменная.смещение), лок: имя.лок.clone() });
                                    }
                                }
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, имена, текущая_процедура, локальные_переменные) {
                            if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                ...
                                let тип_размера_среза = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &размер_среза, временный_размер)?;
                                ...
                                let тип_начала = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &начало_среза, временный_размер)?;
                ...
                "сисвызов" => {
                    ...
                    for аргумент in аргументы {
                        let тип_аргумента = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, аргумент, временный_размер)?;
                ...
                "адрес" => {
                    ...
                    let тип_среза = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &срез, временный_размер)?;
                    match тип_среза {
                        Тип::Срез{..} | Тип::Строка => {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        for (параметр, аргумент) in процедура.параметры.iter().zip(аргументы.iter()).rev() {
                            let тип = скомпилировать_выражение(пп, имена, процедура, локальные_переменные, аргумент, временный_размер)?;
                        ...
                        match &переменная.тип {
                            Тип::Массив{тип_элемента, ..} => {
                                ...
                                пп.код.push(Инструкция{
                                    вид: ВидИнструкции::ГлобальныеДанные(переменная.смещение),
                                    лок: имя.лок.clone()
                                });

                                // СДЕЛАТЬ: проверка входа за границы массива.

                                let тип_индекса = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &индекс, временный_размер)?;
...
fn индексировать_массив(лок: &Лок, переменная: &СкомпПеременная, пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &ЛокальныеПеременные, индекс: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
...
fn найти_переменную_по_имени<'ы>(имя: &str, имена: &'ы Имена, текущая_процедура: &'ы СкомпПроцедура, локальные_переменные: &'ы ЛокальныеПеременные) -> Option<(&'ы СкомпПеременная, Локация)> {
    if let Some(переменная) = локальные_переменные.найти_по_имени(имя) {
...
fn скомпилировать_левое_выражение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &ЛокальныеПеременные, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Вызов{имя, аргументы} => {
            ...
            if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, имена, текущая_процедура, локальные_переменные) {
                ...
                let тип_элемента = индексировать_массив(&имя.лок, переменная, пп, имена, текущая_процедура, локальные_переменные, индекс, временный_размер)?;
        ...
        Выражение::Биноп{ключ, вид, левое, правое} => {
            if let ВидБинопа::Поле = вид {
                let левый_тип = скомпилировать_левое_выражение(пп, имена, текущая_процедура, локальные_переменные, &левое, временный_размер)?;
                match &**правое {
                    Выражение::Идент(имя_поля) => {
                        match &левый_тип {
                            Тип::Структура(имя_структуры) => {
...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &mut ЛокальныеПеременные, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        Утверждение::Присваивание{ключ, левое, правое} => {
            let mut временный_размер = 0;
            let правый_тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, правое, &mut временный_размер)?;
            let левый_тип = скомпилировать_левое_выражение(пп, имена, текущая_процедура, локальные_переменные, левое, &mut временный_размер)?;
            проверить_типы(правое.лок(), &левый_тип, &правый_тип)?;
            if левый_тип.примитивный() {
                записать_примитивный_тип(&ключ.лок, &mut пп.код, левый_тип.размер(&имена.структуры))?;
            } else {
                пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(левый_тип.размер(&имена.структуры)), лок: ключ.лок.clone() });
                    ...
                    match имя.текст.as_str() {
                        ...
                        "печать" => {
                            ...
                            for арг in аргументы {
                                let тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &арг, &mut временный_размер)?;
                                match тип {
                                    Тип::Строка => {
                        ...
                        _ => {
                            if let Some(вызываемая_процедура) = имена.процедуры.get(&имя.текст) {
                                ...
                                for (параметр, аргумент) in вызываемая_процедура.параметры.iter().zip(аргументы.iter()).rev() {
                                    let тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, аргумент, &mut временный_размер)?;
                                    проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                                    match параметр.тип {
                                        Тип::Цел64 | Тип::Лог => {}
                                        _ => {
        ...
        Утверждение::Если{ключ, условие, тело, иначе} => {
            let mut временный_размер = 0;
            let тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &условие, &mut временный_размер)?;
            ...
            локальные_переменные.стек_областей.push(Default::default());
            for утверждение in тело.iter() {
                скомпилировать_утверждение(пп, имена, текущая_процедура, локальные_переменные, утверждение, отложенные_возвраты)?;
            }
            let размер_области = локальные_переменные
                .стек_областей
                .pop()
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                if let Some(результат) = &текущая_процедура.результат {
                    let mut временный_размер = 0;
                    let тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, &выражение, &mut временный_размер)?;
                    ...
                    if тип.примитивный() {
                        ...
                        пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(тип.размер(&имена.структуры)), лок: ключ.лок.clone() });
                        пп.код.push(Инструкция{ вид: ВидИнструкции::СкопироватьПамять, лок: ключ.лок.clone() });
                    }
                    let размер_всех_локальных_переменных = локальные_переменные
                        .стек_областей
                        .iter()
                        .map(|область| область.размер)
                        .sum::<usize>();
        ...
        Утверждение::Декларация{ключ, имя, тип, значение} => {
            let размер_всех_локальных_переменных = локальные_переменные
                .стек_областей
                .iter()
                .map(|область| область.размер)
            ...
            let тип_переменной = скомпилировать_тип(тип, имена)?;
            ...
            if let Some(значение) = значение {
                let mut временный_размер = 0;
                let тип_значения = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_переменные, значение, &mut временный_размер)?;
                ...
                if тип_переменной.примитивный() {
                    записать_примитивный_тип(&ключ.лок, &mut пп.код, тип_переменной.размер(&имена.структуры))?;
                } else {
                    пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(тип_переменной.размер(&имена.структуры)), лок: ключ.лок.clone() });
                    пп.код.push(Инструкция{ вид: ВидИнструкции::СкопироватьПамять, лок: ключ.лок.clone() });
                }
                if временный_размер > 0 {
                    пп.код.push(Инструкция{ вид: ВидИнструкции::ОсвободитьСоСтека(временный_размер as u64), лок: ключ.лок.clone() });
            ...
            let текущая_область_видимости = локальные_переменные
                .стек_областей
                .last_mut()
            ...
            текущая_область_видимости.размер += размер;
...
fn скомпилировать_процедуру(пп: &mut ПП, имена: &mut Имена, процедура: Процедура) -> Результат<()> {
    ...
    for параметр in &процедура.параметры {
        let тип = скомпилировать_тип(&параметр.тип, имена)?;
    ...
    let результат = if let Some(тип) = процедура.тип_результата {
        let результат = РезультатСкомпПроцедуры {
            тип: скомпилировать_тип(&тип, имена)?,
    ...
    match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            for параметр in скомп_процедура.параметры.iter() {
                пп.код.push(Инструкция{ вид: ВидИнструкции::Кадр(параметр.смещение), лок: процедура.имя.лок.clone() });
                if параметр.тип.примитивный() {
                    записать_примитивный_тип(&процедура.имя.лок, &mut пп.код, параметр.тип.размер(&имена.структуры))?;
                } else {
                    пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(параметр.тип.размер(&имена.структуры)), лок: процедура.имя.лок.clone() });
            ...
            let mut локальные_переменные = ЛокальныеПеременные::default();
            локальные_переменные.стек_областей.push(Default::default());
            for утверждение in &блок {
                скомпилировать_утверждение(пп, имена, &скомп_процедура, &mut локальные_переменные, утверждение, &mut отложенные_возвраты)?;
            }
            let размер_области = локальные_переменные
                .стек_областей
                .pop()
...
fn скомпилировать_тип(тип: &Выражение, имена: &Имена) -> Результат<Тип> {
    match тип {
        Выражение::Лог(лексема, _) => {
        ...
        Выражение::Вызов{имя, аргументы} => match имя.текст.as_str() {
            "массив" => {
                ...
                let размер = интерпретировать_выражение_константы(&имена.константы, &размер)?;
                let тип_элемента = Box::new(скомпилировать_тип(&тип_элемента, имена)?);
        ...
        loop {
            ...
            match ключ.вид {
                ВидЛексемы::КлючПер => {
                    ...
                    let тип = скомпилировать_тип(&синтаксис.тип, &self.имена)?;
                ...
                ВидЛексемы::КлючСтрукт => {
                    ...
                    while лекс.подсмотреть_лексему()?.вид != ВидЛексемы::КлючКц {
                        ...
                        let тип = скомпилировать_тип(&Выражение::разобрать(лекс)?, &self.имена)?;
                        let _ = лекс.вытащить_лексему_вида(&[ВидЛексемы::ТочкаЗапятая])?;
