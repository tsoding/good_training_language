    ...
    match утверждение {
        Утверждение::Присваивание{ключ, левое, правое} => {
            ...
            if левый_тип.примитивный() {
                записать_примитивный_тип(&ключ.лок, &mut пп.код, левый_тип.размер())?;
            } else {
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(левый_тип.размер()), лок: ключ.лок.clone() });
