    ...
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(переменная) = имена.переменные.get(&лексема.текст) {
                ...
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьУказатель(переменная.смещение), лок: лексема.лок.clone() });
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер, тип_элемента} = &переменная.тип {
                                ...
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьУказатель(переменная.смещение), лок: имя.лок.clone() });
                        ...
                        match &переменная.тип {
                            Тип::Массив{тип_элемента, ..} => {
                                ...
                                пп.код.push(Инструкция{
                                    вид: ВидИнструкции::ПротолкнутьУказатель(переменная.смещение),
                                    лок: имя.лок.clone()
                                });
...
impl Программа {
        ...
        loop {
            ...
            match ключ.вид {
                ВидЛексемы::КлючПер => {
                    ...
                    let смещение = self.пп.размер_неиниц_данных;
    ...
    pub fn завершить_компиляцию(&mut self) {
        for (_, СкомпПеременная{смещение, ..}) in self.имена.переменные.iter_mut() {
            *смещение += self.пп.иниц_данные.len();
        }
