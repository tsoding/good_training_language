...
pub enum ВидИнструкции {
    ...
    ПротолкнутьЦел(usize), // СДЕЛАТЬ: по возможности, использовать u64 вместо usize для значений пп
...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        Выражение::Лог(лексема, значение) => {
            if *значение {
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(1), лок: лексема.лок.clone() });
            } else {
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(0), лок: лексема.лок.clone() });
            }
            Ok(Тип::Лог)
        }
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(параметр) = текущая_процедура.параметры.iter().find(|параметр| параметр.имя.текст == лексема.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::КадрВторогоСтека(параметр.смещение), лок: лексема.лок.clone() });
                            ...
                            match &левый_тип {
                                Тип::Структура(имя_структуры) => {
                                    ...
                                    if let Some(поле) = структура.поля.get(&имя_поля.текст) {
                                        пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(поле.смещение), лок: ключ.лок.clone() });
    ...
    match выражение {
        ...
        Выражение::Идент(имя) => {
            if let Some(параметр) = текущая_процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::КадрВторогоСтека(параметр.смещение), лок: имя.лок.clone() });
                return Ok(параметр.тип.clone());
            }
    ...
    match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            for параметр in скомп_процедура.параметры.iter() {
                пп.код.push(Инструкция{ вид: ВидИнструкции::КадрВторогоСтека(параметр.смещение), лок: процедура.имя.лок.clone() });
                if параметр.тип.примитивный() {
