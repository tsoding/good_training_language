...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &mut ЛокальныеПеременные, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::Декларация{ключ, имя, тип} => {
            let размер_всех_локальных_переменных = локальные_переменные
                .стек_областей
                .iter()
                .map(|область| область.размер)
                .sum::<usize>();
            let текущая_область_видимости = локальные_переменные.стек_областей.last_mut().expect("Компиляция процедуры обязана создать хотя бы одну область видимости");
            ...
            let тип = скомпилировать_тип(тип, имена)?;
            let размер = выравнять_размер_по_слову(тип.размер(&имена.структуры));
