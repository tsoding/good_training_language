...
fn скомпилировать_левое_выражение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_переменные: &ЛокальныеПеременные, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        Выражение::Лог(лексема, _) => {
            диагностика!(&лексема.лок, "ОШИБКА", "Логической выражение не может находиться в левой части присваивания");
            Err(())
        }
        ...
        Выражение::Идент(имя) => {
            if let Some(переменная) = локальные_переменные.найти_по_имени(&имя.текст) {
            ...
            if let Some(параметр) = текущая_процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::Кадр(параметр.смещение), лок: имя.лок.clone() });
                return Ok(параметр.тип.clone());
            }
