...
use стд::прелюдия::*;
use стд::коллекции::{Вектор, ХэшТаблица};
use super::Результат;
...
pub struct ОбластьВидимости {
    pub переменные: ХэшТаблица<Строка, СкомпПеременная>,
    pub константы: ХэшТаблица<Строка, СкомпКонстанта>,
    pub размер_переменных_на_стеке: usize,
}

#[derive(Default, Clone)]
pub struct ЛокальныеИмена {
    pub стек_областей: Вектор<ОбластьВидимости>,
}
...
pub struct СкомпПроцедура {
    pub имя: Лексема,
    pub параметры: Вектор<СкомпПеременная>,
...
#[derive(Default, Clone)]
pub struct ПП {
    pub код: Вектор<Инструкция>,
    pub иниц_данные: Вектор<u8>,
    pub размер_неиниц_данных: usize,
    pub заплатки_неиниц_указателей: Вектор<usize>,
    pub строки: ХэшТаблица<Строка, usize>,
    pub внешние_символы: ХэшТаблица<Строка, usize>,
    pub библиотеки: ХэшТаблица<Строка, usize>,
}
...
impl ПП {
    pub fn вывалить(&self, точка_входа: usize) {
        ...
        let ширина_столбца_индекса = self.код.len().в_строку().len();
        let mut начало = 0;
...
pub struct Имена {
    pub константы: ХэшТаблица<Строка, СкомпКонстанта>,
    pub процедуры: ХэшТаблица<Строка, СкомпПроцедура>,
    pub переменные: ХэшТаблица<Строка, СкомпПеременная>,
    pub структуры: ХэшТаблица<Строка, Структура>,
}
...
fn вызвать_процедуру(лок: Лок, имена: &Имена, процедура: &СкомпПроцедура, код: &mut Вектор<Инструкция>, временный_размер: &mut usize) -> Результат<()> {
...
fn записать_тип(лок: &Лок, код: &mut Вектор<Инструкция>, структуры: &ХэшТаблица<Строка, Структура>, тип: &Тип) -> Результат<()> {
    if тип.примитивный() {
...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_имена: &mut ЛокальныеИмена, утверждение: &Утверждение, отложенные_возвраты: &mut Вектор<usize>) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::Вилка{ключ, выражение, ветки, любое} => {
            ...
            let mut прыжки_в_конец = Вектор::new();
            for ветка in ветки {
...
fn записать_примитивный_тип(лок: &Лок, код: &mut Вектор<Инструкция>, размер: usize) -> Результат<()> {
    match размер {
...
fn прочитать_примитивный_тип(лок: &Лок, код: &mut Вектор<Инструкция>, структуры: &ХэшТаблица<Строка, Структура>, тип: &Тип) -> Результат<()> {
...
fn скомпилировать_процедуру(пп: &mut ПП, имена: &mut Имена, процедура: Процедура) -> Результат<()> {
    ...
    match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            let mut отложенные_возвраты = Вектор::new();
...
fn скомпилировать_константное_выражение(пп: &ПП, имена: &Имена, локальные_имена: Опция<&ЛокальныеИмена>, выражение: &Выражение) -> Результат<(Вектор<u8>, Тип)> {
    let mut временный_размер = 0;
    let mut пп = (*пп).clone();
    пп.код.clear();
    ...
    let текущая_процедура = СкомпПроцедура {
        имя: Лексема {
            ...
            текст: "<Констатное Выражение>".в_строку(),
        ...
        loop {
            ...
            match ключ.вид {
                ...
                ВидЛексемы::КлючСтрукт => {
                    ...
                    let mut поля: ХэшТаблица<Строка, Поле> = ХэшТаблица::new();
                    let mut размер = 0;
