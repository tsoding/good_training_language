...
pub enum ВидИнструкции {
    ...
    /// Протолкнуть вещественное значение на стек аргументов.
    ПротолкнутьВещ32(f32),
...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, параметры: &Vec<СкомпПараметр>, выражение: &Выражение) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::ВещЧисло(лексема, число) => {
            пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьВещ32(*число), лок: лексема.лок.clone() });
            Ok(Тип::Вещ32)
        },
