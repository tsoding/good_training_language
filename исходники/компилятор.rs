    ...
    match выражение {
        ...
        Выражение::Биноп{ключ, вид, левое, правое} => {
            if let ВидБинопа::Поле = вид {
                let левый_тип = скомпилировать_левое_выражение(пп, имена, текущая_процедура, &левое, временный_размер)?;
                match &**правое {
                    Выражение::Идент(имя_поля) => {
                        match &левый_тип {
                            Тип::Структура(имя_структуры) => {
...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                if let Some(результат) = &текущая_процедура.результат {
                    ...
                    if временный_размер > 0 {
                        пп.код.push(Инструкция{ вид: ВидИнструкции::ОсвободитьСоСтека(временный_размер as u64), лок: выражение.лок().clone() });
                    }
                } else {
        ...
        Утверждение::Декларация{ключ, ..} => {
            сделать!(&ключ.лок, "Компиляция декларации локальных переменных");
            Err(())
        }
    }
}
