...
impl ПП {
    pub fn вывалить(&self, точка_входа: usize) {
        ...
        while начало < self.код.len() {
            ...
            println!("{путь_к_файлу}:{строка}:{столбец}: ");
        ...
        //////////////////////////////
        // for (индекс, Инструкция{вид, лок: Лок{путь_к_файлу, строка, столбец}}) in self.код.iter().enumerate() {
        //     print!("{путь_к_файлу}:{строка}:{столбец}: {индекс:0>ширина_столбца_индекса$}: {вид:?}");
        //     if точка_входа == индекс {
        //         print!(" <- точка входа");
        //     }
        //     println!()
    ...
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some((порядок, параметр)) = параметры.iter().enumerate().find(|(_, параметр)| параметр.имя.текст == лексема.текст) {
                ...
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПрочитатьКадр(порядок), лок: лексема.лок.clone() });
    ...
    match утверждение {
        Утверждение::Присваивание{ключ, имя, значение} => {
            if let Some((порядок, параметр)) = параметры.iter().enumerate().find(|(_, параметр)| параметр.имя.текст == имя.текст) {
                ...
                пп.код.push(Инструкция{ вид: ВидИнструкции::ЗаписатьКадр(порядок), лок: ключ.лок.clone() });
                return Ok(());
            }
            ...
            match имя.текст.as_str() {
                ...
                "печать" => {
                    for арг in аргументы {
                        ...
                        match тип {
                            Тип::Строка => пп.код.push(Инструкция{ вид: ВидИнструкции::ПечатьСтроки, лок: имя.лок.clone() }),
                            Тип::Цел8 => {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        пп.код.push(Инструкция{ вид: ВидИнструкции::СохранитьКадр, лок: имя.лок.clone() });
                        ...
                        пп.код.push(Инструкция{ вид: ВидИнструкции::ВосстановитьКадр, лок: имя.лок.clone() });
                        Ok(())
                    } else {
