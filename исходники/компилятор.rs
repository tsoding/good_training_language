...
struct КонтекстВыражения<'ы> {
    пп: &'ы mut ПП,
    имена: &'ы Имена,
    текущая_процедура: &'ы СкомпПроцедура,
    локальные_имена: &'ы ЛокальныеИмена,
    временный_размер: &'ы mut usize
}

fn найти_переменную_по_имени<'ы>(имя: &str, локальные_имена: &'ы ЛокальныеИмена, текущая_процедура: &'ы СкомпПроцедура, имена: &'ы Имена) -> Option<(&'ы СкомпПеременная, Локация)> {
...
impl<'ы> КонтекстВыражения<'ы> {
    ...
    fn скомпилировать_левое_выражение(&mut self, выражение: &Выражение) -> Результат<Тип> {
        match выражение {
            ...
            Выражение::Идент(имя) => {
                if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                    локация.протолкнуть_смещение(self.пп, &имя.лок, переменная.смещение);
            ...
            Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
                Выражение::Идент(имя) => {
                    if аргументы.len() != 1 {
                    ...
                    if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                        локация.протолкнуть_смещение(self.пп, &имя.лок, переменная.смещение);
                        let тип_элемента = self.индексировать_массив(&имя.лок, переменная, индекс)?;
                ...
                _ => {
                    сделать!(вызываемое.лок(), "Вызов сложных выражений в левой части присваивания");
                    Err(())
                }
            }
            ...
            Выражение::Биноп{ключ, вид, левое, правое} => {
                if let ВидБинопа::Поле = вид {
                    let левый_тип = self.скомпилировать_левое_выражение(&левое)?;
                    match &**правое {
                        Выражение::Идент(имя_поля) => {
                            match &левый_тип {
                                Тип::Структура(имя_структуры) => {
                                    let структура = self.имена
                                        .структуры
                                        .get(имя_структуры)
                                    ...
                                    if let Some(поле) = структура.поля.get(&имя_поля.текст) {
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(поле.смещение), лок: ключ.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: ключ.лок.clone() });
                                        Ok(поле.тип.clone())
                                    } else {
    ...
    fn скомпилировать_выражение(&mut self, выражение: &Выражение) -> Результат<Тип> {
        match выражение {
            Выражение::Лог(лексема, значение) => {
                if *значение {
                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(1), лок: лексема.лок.clone() });
                } else {
                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(0), лок: лексема.лок.clone() });
                }
                Ok(Тип::Лог)
            }
            Выражение::ЦелЧисло(лексема, число) => {
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(*число), лок: лексема.лок.clone() });
                Ok(Тип::Цел64)
            },
            ...
            Выражение::Строка(строка) => {
                let (указатель, длинна) = if let Some(указатель) = self.пп.строки.get(&строка.текст) {
                    (*указатель, строка.текст.len())
                } else {
                    let указатель = self.пп.иниц_данные.len();
                    let длинна = строка.текст.len();
                    self.пп.иниц_данные.extend(строка.текст.as_bytes());
                    self.пп.иниц_данные.push(0); // ЗАМЕТКА: нулл-терминатор на случае если мы захотим что-то передать в сишный код
                    self.пп.строки.insert(строка.текст.clone(), указатель);
                    (указатель, длинна)
                };
                let тип = Тип::Строка;
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВыделитьНаСтеке(тип.размер(&self.имена.структуры) as u64), лок: строка.лок.clone() });
                *self.временный_размер += тип.размер(&self.имена.структуры);
                ...
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(длинна), лок: строка.лок.clone() });
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(СРЕЗ_РАЗМЕР_СМЕЩЕНИЕ as i32), лок: строка.лок.clone() });
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: строка.лок.clone() });

                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ГлобальныеДанные(указатель as i32), лок: строка.лок.clone() });
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(СРЕЗ_АДРЕС_СМЕЩЕНИЕ as i32), лок: строка.лок.clone() });
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: строка.лок.clone() });

                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(0), лок: строка.лок.clone() });

                Ok(тип)
            }
            Выражение::Идент(лексема) => {
                if let Some(константа) = найти_константу_по_имени(&лексема.текст, self.имена, Some(self.локальные_имена)) {
                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(константа.значение), лок: лексема.лок.clone() });
                    return Ok(Тип::Цел64);
                }
                if let Some((переменная, локация)) = найти_переменную_по_имени(&лексема.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                    локация.протолкнуть_смещение(self.пп, &лексема.лок, переменная.смещение);
                    if переменная.тип.примитивный() {
                        прочитать_примитивный_тип(&лексема.лок, &mut self.пп.код, переменная.тип.размер(&self.имена.структуры))?;
                    }
                    return Ok(переменная.тип.clone());
                }
            ...
            Выражение::Биноп {ключ, вид, левое, правое} => {
                match вид {
                                    ...
                                    Тип::Структура(имя_структуры) => {
                                        ...
                                        if let Some(поле) = структура.поля.get(&имя_поля.текст) {
                                            ...
                                            if поле.тип.примитивный() {
                                                прочитать_примитивный_тип(&ключ.лок, &mut self.пп.код, поле.тип.размер(&self.имена.структуры))?;
                                            }
                                            Ok(поле.тип.clone())
                                        } else {
                                            диагностика!(&ключ.лок, "ОШИБКА", "У структуры «{тип}» отстутствует поле «{поле}»", тип = левый_тип.текст(), поле = имя_поля.текст);
                    ...
                    ВидБинопа::Меньше => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелМеньше, лок: ключ.лок.clone() });
                            }
                            Тип::Вещ32 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Меньше, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::Больше => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелБольше, лок: ключ.лок.clone() });
                            }
                            Тип::Вещ32 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Больше, лок: ключ.лок.clone() });
                            }
                            _ => {
                                диагностика!(&ключ.лок, "ОШИБКА", "Меньше для типов «{тип}» не поддерживается",
                                             тип = левый_тип.текст());
                    ...
                    ВидБинопа::Или => {
                        ...
                        match левый_тип {
                            Тип::Лог | Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::БитИли, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::И => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::БитИ, лок: ключ.лок.clone() });
                            }
                            Тип::Лог => {
                    ...
                    ВидБинопа::Сложение => {
                        ...
                        match левый_тип {
                            ...
                            Тип::Вещ32 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Сложение, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::Вычитание => {
                        ...
                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелВычитание, лок: ключ.лок.clone() });
                        Ok(Тип::Цел64)
                    }
                    ВидБинопа::Умножение => {
                        let левый_тип = self.скомпилировать_выражение(&левое)?;
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: ключ.лок.clone() });
                            }
                            Тип::Вещ32 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Умножение, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::Деление => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелДеление, лок: ключ.лок.clone() });
                            }
                            Тип::Вещ32 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Деление, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::Остаток => {
                        ...
                        проверить_типы(левое.лок(), &Тип::Цел64, &левый_тип)?;
                        проверить_типы(правое.лок(), &Тип::Цел64, &правый_тип)?;
                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелОстаток, лок: ключ.лок.clone() });
                        Ok(Тип::Цел64)
                    }
                    ВидБинопа::Равно => {
                        let левый_тип = self.скомпилировать_выражение(&левое)?;
                        ...
                        проверить_типы(правое.лок(), &левый_тип, &правый_тип)?;
                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелРавно, лок: ключ.лок.clone() });
                        Ok(Тип::Лог)
                    }
                    ВидБинопа::Как => {
                        let значение = левое;
                        let тип = правое;
                        let тип_цели = скомпилировать_тип(тип, self.имена, Some(self.локальные_имена))?;
                        let тип_значения = self.скомпилировать_выражение(&значение)?;
                        ...
                        match (&тип_значения, &тип_цели) {
                            ...
                            (Тип::Цел64, Тип::Вещ32) => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::КонвертЦел64Вещ32, лок: ключ.лок.clone() });
                                Ok(тип_цели)
                            }
                            (Тип::Вещ32, Тип::Цел64) => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::КонвертВещ32Цел64, лок: ключ.лок.clone() });
                                Ok(тип_цели)
                            }
                            _ => {
                    ...
                    ВидБинопа::ЛевоеБитовоеСмещение => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::БитСмещениеВлево, лок: ключ.лок.clone() });
                            }
                            _ => {
                    ...
                    ВидБинопа::ПравоеБитовоеСмещение => {
                        ...
                        match левый_тип {
                            Тип::Цел64 => {
                                self.пп.код.push(Инструкция{ вид: ВидИнструкции::БитСмещениеВправо, лок: ключ.лок.clone() });
                            }
                            _ => {
            ...
            Выражение::УнарныйМинус {ключ, выражение} => {
                let тип = self.скомпилировать_выражение(&выражение)?;
                match тип {
                    Тип::Вещ32 => {
                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Вещ32Инверт, лок: ключ.лок.clone() });
                        Ok(тип)
                    }
                    _ => {
            ...
            Выражение::Вызов {вызываемое, аргументы} => {
                match &**вызываемое {
                    Выражение::Идент(имя) => match имя.текст.as_str() {
                        ...
                        "ввод" => {
                            ...
                            if let Выражение::Идент(имя) = массив {
                                if let Some(_параметр) = self.текущая_процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                                ...
                                if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                                    if let Тип::Массив{размер, тип_элемента} = &переменная.тип {
                                        ...
                                        локация.протолкнуть_смещение(self.пп, &имя.лок, переменная.смещение);
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(*размер), лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Ввод, лок: имя.лок.clone() });
                                        return Ok(Тип::Цел64);
                                    } else {
                            ...
                            if let Выражение::Идент(имя) = массив {
                                ...
                                if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                                    if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                        ...
                                        let тип_размера_среза = self.скомпилировать_выражение(&размер_среза)?;
                                        проверить_типы(&размер_среза.лок(), &Тип::Цел64, &тип_размера_среза)?;
                                        локация.протолкнуть_смещение(self.пп, &имя.лок, переменная.смещение);

                                        let тип_начала = self.скомпилировать_выражение(&начало_среза)?;
                                        проверить_типы(&размер_среза.лок(), &Тип::Цел64, &тип_начала)?;

                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(тип_элемента.размер(&self.имена.структуры)), лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: имя.лок.clone() });

                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВыделитьНаСтеке(тип_среза.размер(&self.имена.структуры) as u64), лок: имя.лок.clone() });
                                        *self.временный_размер += тип_среза.размер(&self.имена.структуры);
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(СРЕЗ_АДРЕС_СМЕЩЕНИЕ as i32), лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(СРЕЗ_РАЗМЕР_СМЕЩЕНИЕ as i32), лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: имя.лок.clone() });

                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ВершинаСтека(0), лок: имя.лок.clone() });
                        ...
                        "сисвызов" => {
                            ...
                            for аргумент in аргументы {
                                let тип_аргумента = self.скомпилировать_выражение(аргумент)?;
                                проверить_типы(&аргумент.лок(), &Тип::Цел64, &тип_аргумента)?;
                            }

                            self.пп.код.push(Инструкция{ вид: ВидИнструкции::СисВызов{арность}, лок: имя.лок.clone() });
                            Ok(Тип::Цел64)
                        }
                        "адрес" => {
                            ...
                            let тип_среза = self.скомпилировать_выражение(&срез)?;
                            match тип_среза {
                                Тип::Срез{..} | Тип::Строка => {
                                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(СРЕЗ_АДРЕС_СМЕЩЕНИЕ), лок: имя.лок.clone()});
                                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: имя.лок.clone()});
                                    self.пп.код.push(Инструкция{ вид: ВидИнструкции::Прочитать64, лок: имя.лок.clone()});
                                    Ok(Тип::Цел64)
                                }
                                _ => {
                        ...
                        _ => {
                            if let Some(процедура) = self.имена.процедуры.get(&имя.текст) {
                                ...
                                for (параметр, аргумент) in процедура.параметры.iter().zip(аргументы.iter()).rev() {
                                    let тип = self.скомпилировать_выражение(аргумент)?;
                                    проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                                }
                                вызвать_процедуру(имя.лок.clone(), self.имена, &процедура, &mut self.пп.код, self.временный_размер)?;
                            ...
                            if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, self.локальные_имена, self.текущая_процедура, self.имена) {
                                match &переменная.тип {
                                    Тип::Массив{тип_элемента, ..} => {
                                        ...
                                        let тип_индекса = self.скомпилировать_выражение(&индекс)?;
                                        проверить_типы(&индекс.лок(), &Тип::Цел64, &тип_индекса)?;
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(тип_элемента.размер(&self.имена.структуры)), лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: имя.лок.clone() });
                                        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: имя.лок.clone() });
                                        if тип_элемента.примитивный() {
                                            прочитать_примитивный_тип(&имя.лок, &mut self.пп.код, тип_элемента.размер(&self.имена.структуры))?;
                                        }
                    ...
                    _ => {
                        let тип = self.скомпилировать_выражение(вызываемое)?;
                        сделать!(вызываемое.лок(), "Компиляция вызовов сложных выражений типа «{тип}»", тип = тип.текст());
                        return Err(())
                    }
                }
            }
            ...
            Выражение::Отрицание{ключ, выражение} => {
                ...
                self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЛогОтрицание, лок: ключ.лок.clone() });
                Ok(Тип::Лог)
            }
        }
    }

    fn индексировать_массив(&mut self, лок: &Лок, переменная: &СкомпПеременная, индекс: &Выражение) -> Результат<Тип> {
        ...
        проверить_типы(&индекс.лок(), &Тип::Цел64, &тип_индекса)?;
        self.пп.код.push(Инструкция{ вид: ВидИнструкции::Целое(тип_элемента.размер(&self.имена.структуры)), лок: индекс.лок().clone() });
        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: индекс.лок().clone() });
        self.пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: индекс.лок().clone() });
...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_имена: &mut ЛокальныеИмена, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        Утверждение::Присваивание{ключ, левое, правое} => {
            ...
            let mut контекст = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер };
            let правый_тип = контекст.скомпилировать_выражение(правое)?;
            let левый_тип = контекст.скомпилировать_левое_выражение(левое)?;
            проверить_типы(правое.лок(), &левый_тип, &правый_тип)?;
        ...
        Утверждение::Выражение{выражение} => {
            match выражение {
                Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
                        ...
                        match имя.текст.as_str() {
                            ...
                            "печать" => {
                                ...
                                for арг in аргументы {
                                    let тип = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(&арг)?;
                                    match тип {
                                        Тип::Строка => {
                            ...
                            _ => {
                                if let Some(вызываемая_процедура) = имена.процедуры.get(&имя.текст) {
                                    ...
                                    for (параметр, аргумент) in вызываемая_процедура.параметры.iter().zip(аргументы.iter()).rev() {
                                        let тип = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(аргумент)?;
                                        проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                                    }
                    ...
                    _ => {
                        сделать!(вызываемое.лок(), "Вызов сложных выражений в контексте утверждений");
                        Err(())
                    }
                }
                _ => {
        ...
        Утверждение::Если{ключ, условие, тело, иначе} => {
            let mut временный_размер = 0;
            let тип = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(&условие)?;
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                if let Some(результат) = &текущая_процедура.результат {
                    let mut временный_размер = 0;
                    let тип = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(&выражение)?;
        ...
        Утверждение::Для{ключ, индекс, свой_индекс, нижняя_граница, верхняя_граница, тело} => {
            ...
            if let Some((переменная, локация)) = найти_переменную_по_имени(&индекс.текст, локальные_имена, текущая_процедура, имена) {
                проверить_типы(&индекс.лок, &Тип::Цел64, &переменная.тип)?;

                // Инициализация индексной переменной
                let mut временный_размер = 0;
                let тип_нижней_границы = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(&нижняя_граница)?;
                ...
                let тип_верхней_границы = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(&верхняя_граница)?;
        ...
        Утверждение::ДекларацияПеременной{ключ, имя, тип, значение} => {
            ...
            if let Some(значение) = значение {
                let mut временный_размер = 0;
                let тип_значения = КонтекстВыражения { пп, имена, текущая_процедура, локальные_имена, временный_размер: &mut временный_размер }.скомпилировать_выражение(значение)?;
...
fn интерпретировать_выражение_константы(имена: &Имена, локальные_имена: Option<&ЛокальныеИмена>, выражение: &Выражение) -> Результат<usize> {
    match выражение {
        ...
        Выражение::Вызов{вызываемое, ..} => {
            сделать!(вызываемое.лок(), "вызов функции в константном контексте");
            Err(())
        }
    }
}
...
fn скомпилировать_тип(тип: &Выражение, имена: &Имена, локальные_имена: Option<&ЛокальныеИмена>) -> Результат<Тип> {
    match тип {
        ...
        Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
            ...
            _ => {
                сделать!(вызываемое.лок(), "Вызов сложных выражений в контексте типов");
                Err(())
            }
        }
