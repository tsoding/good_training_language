    ...
    match выражение {
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            match вид {
                ...
                ВидБинопа::МеньшеРавно => {
                    сделать!(&ключ.лок, "Реализовать биноп МеньшеРавно");
                    return Err(());
                }
                ВидБинопа::БольшеРавно => {
                    сделать!(&ключ.лок, "Реализовать биноп БольшеРавно");
                    return Err(());
                }
                ВидБинопа::Меньше => {
                    let левый_тип = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_имена, &левое, временный_размер)?;
                ...
                ВидБинопа::И => {
                    ...
                    match левый_тип {
                        ...
                        Тип::Лог => {
                            пп.код.push(Инструкция{ вид: ВидИнструкции::ЛогИ, лок: ключ.лок.clone() });
                        }
                        _ => {
                ...
                ВидБинопа::Вычитание => {
                    ...
                    match левый_тип {
                        ...
                        Тип::Вещ32 => {
                            сделать!(левое.лок(), "Вычитание для вещественных типов");
                            return Err(());
                        }
                        _ => {
                ...
                ВидБинопа::Остаток => {
                    ...
                    match левый_тип {
                        ...
                        Тип::Нат64 => пп.код.push(Инструкция{ вид: ВидИнструкции::НатОстаток,   лок: ключ.лок.clone() }),
                        Тип::Вещ32 => {
                            сделать!(&ключ.лок, "Остаток от деления вещественных чисел. Что-то типа fmod");
                            return Err(());
                        }
                        _ => {
                ...
                ВидБинопа::Как => {
                    ...
                    match (&тип_значения, &тип_цели) {
                        (Тип::Нат64, Тип::Нат8) => Ok(тип_цели),
                        (Тип::Нат64, Тип::Цел64) => Ok(тип_цели),
                        (Тип::Цел64, Тип::Нат64) => Ok(тип_цели),
                        (Тип::Нат8, Тип::Нат64) => Ok(тип_цели),
                        (Тип::Цел64, Тип::Вещ32) => {
        ...
        Выражение::УнарныйМинус {ключ, выражение} => {
            ...
            match тип {
                ...
                _ => {
                    // СДЕЛАТЬ: поддержка унарного минус для всех нужных типов
        ...
        Выражение::Вызов {вызываемое, аргументы} => {
            match &**вызываемое {
                ...
                _ => {
                    let тип_вызываемого = скомпилировать_выражение(пп, имена, текущая_процедура, локальные_имена, вызываемое, временный_размер)?;
                    match &тип_вызываемого {
                        Тип::Массив{тип_элемента, ..} => {
                            let [индекс] = &аргументы[..] else {
                                диагностика!(вызываемое.лок(), "ОШИБКА", "Некорректная размерность массива. Ожидался 1 индекс, но повстречалось {ранк}", ранк = аргументы.len());
                                return Err(());
                            };

                            индексировать_массив(тип_элемента, пп, имена, текущая_процедура, локальные_имена, индекс, временный_размер)?;
                            if тип_элемента.примитивный() {
                                прочитать_примитивный_тип(вызываемое.лок(), &mut пп.код, &имена.структуры, &тип_элемента)?;
                            }
                            Ok(*тип_элемента.clone())
                        }
                        Тип::Срез{..} => {
                            сделать!(вызываемое.лок(), "Индексация срезов из сложных выражений");
                            Err(())
                        }
                        _ => {
                            диагностика!(вызываемое.лок(), "ОШИБКА", "Невозможно индексировать тип «{тип}»", тип = тип_вызываемого.текст());
                            Err(())
                        }
                    }
                }
...
fn скомпилировать_левое_выражение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_имена: &ЛокальныеИмена, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
            Выражение::Идент(имя) => {
                if let Some((переменная, локация)) = найти_переменную_по_имени(&имя.текст, имена, текущая_процедура, локальные_имена) {
                    match &переменная.тип {
                        ...
                        _ => {
                            диагностика!(&имя.лок, "ОШИБКА", "Не возможно индексировать переменные тип «{тип}»", тип = переменная.тип.текст());
                            return Err(());
                        }
                    }
                }
            ...
            _ => {
                let тип_вызываемого = скомпилировать_левое_выражение(пп, имена, текущая_процедура, локальные_имена, вызываемое, временный_размер)?;
                match &тип_вызываемого {
                    Тип::Массив{тип_элемента, ..} => {
                        if аргументы.len() != 1 {
                            диагностика!(вызываемое.лок(), "ОШИБКА", "Некорректная размерность массива. Ожидался 1 индекс, но повстречалось {ранк}", ранк = аргументы.len());
                    ...
                    Тип::Срез{..} => {
                        сделать!(вызываемое.лок(), "Индексация срезов в левой части присваивания");
                        Err(())
                    }
                    _ => {
                        диагностика!(вызываемое.лок(), "ОШИБКА", "Не возможно индексировать переменные тип «{тип}»", тип = тип_вызываемого.текст());
                        Err(())
                    }
                }
            }
...
fn скомпилировать_тип(пп: &ПП, тип: &Выражение, имена: &Имена, локальные_имена: Option<&ЛокальныеИмена>) -> Результат<Тип> {
    match тип {
        ...
        Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
            Выражение::Идент(имя) => match имя.текст.as_str() {
                "массив" => {
                    let [выражение_размера, тип_элемента] = аргументы.as_slice() else {
