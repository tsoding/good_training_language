...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, параметры: &Vec<СкомпПараметр>, выражение: &Выражение) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(параметр) = параметры.iter().find(|(_, параметр)| параметр.имя.текст == лексема.текст) {
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                ...
                match тип {
                    ...
                    _ => {
                        сделать!(&выражение.лок(), "Возврат типа «{тип}»", тип = тип.текст());
                        return Err(())
                    }
                }
            }
...
fn скомпилировать_процедуру(пп: &mut ПП, имена: &Имена, процедура: Процедура) -> Результат<СкомпПроцедура> {
    ...
    let точка_входа = match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            for параметр in параметры.iter() {
                ...
                if параметр.тип.примитивный() {
                    match параметр.тип.размер() {
                        1 => пп.код.push(Инструкция{ вид: ВидИнструкции::Записать8, лок: процедура.имя.лок.clone() }),
                        2 => сделать!(&процедура.имя.лок, "Запись параметров размером в 16 бит"),
                        4 => пп.код.push(Инструкция{ вид: ВидИнструкции::Записать32, лок: процедура.имя.лок.clone() }),
                        8 => пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: процедура.имя.лок.clone() }),
            ...
            if let Some(результат) = &результат {
                ...
                if результат.тип.примитивный() {
                    match результат.тип.размер() {
                        1 => сделать!(&процедура.имя.лок, "Чтение параметров размером в 8 бит"),
                        2 => сделать!(&процедура.имя.лок, "Чтение параметров размером в 16 бит"),
                        4 => пп.код.push(Инструкция{ вид: ВидИнструкции::Прочитать32, лок: процедура.имя.лок.clone() }),
                        8 => пп.код.push(Инструкция{ вид: ВидИнструкции::Прочитать64, лок: процедура.имя.лок.clone() }),
                        размер => unreachable!("Примитивных типов размера {} нет", размер),
                    }
