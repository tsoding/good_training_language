...
pub enum ВидИнструкции {
    ...
    ВызватьВнешнююПроцедуру{индекс: usize, арность: usize, результат: bool},
            ...
            match имя.текст.as_str() {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        if тип_результата != Тип::Цел64 && тип_результата != Тип::Лог {
...
fn вызвать_процедуру(лок: Лок, процедура: &СкомпПроцедура, код: &mut Vec<Инструкция>) -> Результат<()> {
    match &процедура.точка_входа {
        ...
        &ТочкаВхода::Внешняя{индекс} => {
            ...
            let результат = match &процедура.тип_результата {
                Some(Тип::Лог) | Some(Тип::Цел64) => true,
                Some(тип) => {
                    сделать!(&лок, "Возврат типа «{тип}» из внешних процедур", тип = тип.текст());
                    return Err(())
                }
                None => false,
            };
