    ...
    match выражение {
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some((порядок, параметр)) = параметры.iter().enumerate().find(|(_, параметр)| параметр.имя.текст == лексема.текст) {
                ...
                пп.код.push(Инструкция::ПротолкнутьОтКадра(порядок));
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            ...
            match вид {
                ...
                ВидБинопа::Равно => {
                    if левый_тип != Тип::ПримТип(ПримТип::Цел8) {
...
fn скомпилировать_утвержление(пп: &mut ПП, имена: &Имена, параметры: &Vec<СкомпПараметр>, утверждение: &Утверждение) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::ПрисваиваниеМассива{имя, индекс, значение, ..} => {
            ...
            let (прим_тип, _размер) = if let Тип::Массив{прим_тип, размер} = переменная.тип.clone() {
                (прим_тип, размер)
            } else {
            ...
            if прим_тип != ПримТип::Цел1 {
                сделать!(&имя.лок, "Мы пока не умеем присваивать массивы элементов типа {тип}", тип = прим_тип.текст());
                return Err(());
            }

            let тип_значение = скомпилировать_выражение(пп, имена, параметры, значение)?;
            проверить_типы(значение.лок(), &Тип::ПримТип(прим_тип.clone()), &тип_значение)?;

            пп.заплатки_неиниц_указателей.push(пп.код.len());
            пп.код.push(Инструкция::ПротолкнутьУказатель(переменная.адрес));

            let тип_индекса = скомпилировать_выражение(пп, имена, параметры, индекс)?;
            проверить_типы(&индекс.лок(), &Тип::ПримТип(ПримТип::Цел8), &тип_индекса)?;
            сделать!(&имя.лок, "Компиляция присваивания массива.");

            // СДЕЛАТЬ: проверку входа за границы массива.

            пп.код.push(Инструкция::ПротолкнутьЦелое(прим_тип.размер()));
...
fn скомпилировать_параметр(константы: &HashMap<String, СкомпКонстанта>, параметр: &Параметр) -> Результат<СкомпПараметр> {
    Ok(СкомпПараметр {
        ...
        тип: скомпилировать_тип(параметр.тип.clone(), константы)?
    })
}
