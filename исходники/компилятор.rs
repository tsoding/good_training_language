...
pub enum ВидИнструкции {
    ...
    // для ВызватьПроцедуру.
    ВызватьВнутреннююПроцедуру(usize),
    ВызватьВнешнююПроцедуру(usize),
    Прыжок(usize),
...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, параметры: &Vec<СкомпПараметр>, выражение: &Выражение) -> Результат<Тип> {
    match выражение {
        Выражение::Число(лексема, число) => {
            пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦелое(*число), лок: лексема.лок.clone() });
            Ok(Тип::Цел64)
        },
        Выражение::Строка(строка) => {
            let (указатель, длинна) = if let Some(указатель) = пп.строки.get(&строка.текст) {
                ...
                // СДЕЛАТЬ: нулл-терминировать строковые литералы
                пп.строки.insert(строка.текст.clone(), указатель);
                (указатель, длинна)
            };
            пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦелое(длинна),        лок: строка.лок.clone() });
        ...
        Выражение::Идент(лексема) => {
            if let Some(константа) = имена.константы.get(&лексема.текст) {
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦелое(константа.значение), лок: лексема.лок.clone() });
                return Ok(Тип::Цел64);
            }
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            match вид {
                ...
                ВидБинопа::Как => {
                    ...
                    if !(тип_цели == Тип::Цел8 && тип_значения == Тип::Цел64) {
        ...
        Выражение::Вызов {имя, аргументы} => {
            match имя.текст.as_str() {
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер, тип_элемента} = &переменная.тип {
                                ...
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦелое(*размер), лок: имя.лок.clone() });
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                ...
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦелое(тип_элемента.размер()), лок: имя.лок.clone() });
                ...
                "адрес" => {
                    сделать!(&имя.лок, "Компиляция интринсика «адрес»");
                    Err(())
                }
                _ => {
                    сделать!(&имя.лок, "Компиляция вызова выражения пользовательской процедуры");
                    // У нас пока нет процедур, которые умеют возвращать значение, поэтому это особо и не нужно.
                    Err(())
                }
            }
        }
        ...
        Выражение::Отрицание{ключ, ..} => {
            сделать!(&ключ.лок, "компиляция выражения отрицания");
            return Err(())
        }
    }
}
...
fn вызвать_точку_входа(лок: Лок, точка_входа: &ТочкаВхода, код: &mut Vec<Инструкция>) {
    match точка_входа {
        ...
        &ТочкаВхода::Внешняя{индекс} => {
            код.push(Инструкция {
                вид: ВидИнструкции::ВызватьВнешнююПроцедуру(индекс),
                лок,
            });
        },
    }
    ...
    match утверждение {
        Утверждение::Присваивание{ключ, имя, значение} => {
            ...
            if let Some(переменная) = имена.переменные.get(имя.текст.as_str()) {
                ...
                пп.заплатки_неиниц_указателей.push(пп.код.len());
                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьУказатель(переменная.адрес), лок: имя.лок.clone() });
                пп.код.push(Инструкция{ вид: ВидИнструкции::Записать64, лок: ключ.лок.clone() });
                return Ok(())
            }
            ...
            match имя.текст.as_str() {
                    ...
                    for арг in аргументы {
                        ...
                        match тип {
                            ...
                            Тип::Цел64 => {
                                ...
                                вызвать_точку_входа(арг.лок().clone(), &процедура.точка_входа, &mut пп.код);
                                пп.код.push(Инструкция { вид: ВидИнструкции::Вытолкнуть(процедура.параметры.len()), лок: арг.лок().clone() });
                            }
                            Тип::Цел8 => {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        вызвать_точку_входа(имя.лок.clone(), &процедура.точка_входа, &mut пп.код);
