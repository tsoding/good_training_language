...
pub struct ПП {
    ...
    pub инициализированные_данные: Vec<u8>,
}
...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, выражение: &Выражение) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Строка(строка) => {
            let указатель = пп.инициализированные_данные.len();
            let длинна = строка.текст.len();
            пп.инициализированные_данные.extend(строка.текст.as_bytes());
        ...
        loop {
            ...
            match ключ.вид {
                ВидЛексемы::КлючПер => {
                    ...
                    let адрес = self.пп.инициализированные_данные.len();
                    let новый_размер = self.пп.инициализированные_данные.len() + синтаксис.тип.размер();
                    self.пп.инициализированные_данные.resize(новый_размер, 0u8);
