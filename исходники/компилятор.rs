...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, параметры: &Vec<СкомпПараметр>, выражение: &Выражение, временный_размер: &mut usize) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Строка(строка) => {
            ...
            let тип = Тип::Строка;
            пп.код.push(Инструкция{ вид: ВидИнструкции::ВыделитьНаВторомСтеке(тип.размер() as u64), лок: строка.лок.clone() });
            *временный_размер += тип.размер();
        ...
        Выражение::Идент(лексема) => {
            ...
            if let Some(параметр) = параметры.iter().find(|параметр| параметр.имя.текст == лексема.текст) {
                ...
                if параметр.тип.примитивный() {
                    прочитать_примитивный_тип(&лексема.лок, &mut пп.код, параметр.тип.размер())?;
                }
        ...
        Выражение::Биноп {ключ, вид, левое, правое} => {
            match вид {
                ВидБинопа::Меньше => {
                    let левый_тип = скомпилировать_выражение(пп, имена, параметры, &левое, временный_размер)?;
                    let правый_тип = скомпилировать_выражение(пп, имена, параметры, &правое, временный_размер)?;
                ...
                ВидБинопа::Как => {
                    ...
                    let тип_значения = скомпилировать_выражение(пп, имена, параметры, &значение, временный_размер)?;
        ...
        Выражение::УнарныйМинус {ключ, выражение} => {
            let тип = скомпилировать_выражение(пп, имена, параметры, &выражение, временный_размер)?;
            match тип {
                Тип::Вещ32 => {
            ...
            match имя.текст.as_str() {
                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                ...
                                let тип_размера_среза = скомпилировать_выражение(пп, имена, параметры, &размер_среза, временный_размер)?;
                                проверить_типы(&размер_среза.лок(), &Тип::Цел64, &тип_размера_среза)?;

                                пп.заплатки_неиниц_указателей.push(пп.код.len());
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьУказатель(переменная.адрес), лок: имя.лок.clone() });

                                let тип_начала = скомпилировать_выражение(пп, имена, параметры, &начало_среза, временный_размер)?;
                                ...
                                пп.код.push(Инструкция{ вид: ВидИнструкции::ВыделитьНаВторомСтеке(тип_среза.размер() as u64), лок: имя.лок.clone() });
                                *временный_размер += тип_среза.размер();
                ...
                "сисвызов" => {
                    ...
                    for аргумент in аргументы {
                        let тип_аргумента = скомпилировать_выражение(пп, имена, параметры, аргумент, временный_размер)?;
                ...
                "адрес" => {
                    ...
                    let тип_среза = скомпилировать_выражение(пп, имена, параметры, &срез, временный_размер)?;
                    match тип_среза {
                        Тип::Срез{..} | Тип::Строка => {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        ...
                        for (параметр, аргумент) in процедура.параметры.iter().zip(аргументы.iter()).rev() {
                            let тип = скомпилировать_выражение(пп, имена, параметры, аргумент, временный_размер)?;
                            проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                        }
                        вызвать_процедуру(имя.лок.clone(), &процедура, &mut пп.код, временный_размер)?;
                    ...
                    if let Some(переменная) = имена.переменные.get(&имя.текст) {
                        match &переменная.тип {
                            Тип::Массив{тип_элемента, ..} => {
                                ...
                                let тип_индекса = скомпилировать_выражение(пп, имена, параметры, &индекс, временный_размер)?;
...
fn вызвать_процедуру(лок: Лок, процедура: &СкомпПроцедура, код: &mut Vec<Инструкция>, временный_размер: &mut usize) -> Результат<()> {
        ...
        &ТочкаВхода::Внутреняя{адрес} => {
            ...
            if let Some(результат) = &процедура.результат {
                if !результат.тип.примитивный() {
                    *временный_размер += результат.тип.размер();
                }
            }
        },
...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, процедура: &СкомпПроцедура, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        Утверждение::Присваивание{ключ, имя, значение} => {
            if let Some(параметр) = процедура.параметры.iter().find(|параметр| параметр.имя.текст == имя.текст) {
                let mut временный_размер = 0;
                let тип_значения = скомпилировать_выражение(пп, имена, &процедура.параметры, &значение, &mut временный_размер)?;
                проверить_типы(&значение.лок(), &параметр.тип, &тип_значения)?;
                пп.код.push(Инструкция{ вид: ВидИнструкции::КадрВторогоСтека(параметр.смещение_от_кадра), лок: ключ.лок.clone() });
                if тип_значения.примитивный() {
                    записать_примитивный_тип(&ключ.лок, &mut пп.код, тип_значения.размер())?;
                } else {
                    пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(тип_значения.размер()), лок: ключ.лок.clone() });
            ...
            if let Some(переменная) = имена.переменные.get(имя.текст.as_str()) {
                let mut временный_размер = 0;
                let тип = скомпилировать_выражение(пп, имена, &процедура.параметры, &значение, &mut временный_размер)?;
        ...
        Утверждение::ПрисваиваниеМассива{ключ, имя, индекс, значение} => {
            ...
            let тип_индекса = скомпилировать_выражение(пп, имена, &процедура.параметры, индекс, &mut временный_размер)?;
            проверить_типы(&индекс.лок(), &Тип::Цел64, &тип_индекса)?;
            пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(тип_элемента.размер()), лок: имя.лок.clone() });
            пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелУмножение, лок: имя.лок.clone() });
            пп.код.push(Инструкция{ вид: ВидИнструкции::ЦелСложение, лок: имя.лок.clone() });
            match *тип_элемента {
                Тип::Цел8 => пп.код.push(Инструкция{ вид: ВидИнструкции::Записать8, лок: ключ.лок.clone() }),
                Тип::Вещ32 => пп.код.push(Инструкция{ вид: ВидИнструкции::Записать32, лок: ключ.лок.clone() }),
                _ => {
            ...
            match имя.текст.as_str() {
                ...
                "печать" => {
                    ...
                    for арг in аргументы {
                        let тип = скомпилировать_выражение(пп, имена, &процедура.параметры, &арг, &mut временный_размер)?;
                        match тип {
                            Тип::Строка => {
                            ...
                            Тип::Цел64 => {
                                ...
                                вызвать_процедуру(арг.лок().clone(), процедура, &mut пп.код, &mut временный_размер)?;
                            }
                            Тип::Лог => {
                ...
                _ => {
                    if let Some(процедура) = имена.процедуры.get(&имя.текст) {
                        if процедура.результат.is_some() {
                        ...
                        let mut временный_размер = 0;
                        for (параметр, аргумент) in процедура.параметры.iter().zip(аргументы.iter()).rev() {
                            let тип = скомпилировать_выражение(пп, имена, &процедура.параметры, аргумент, &mut временный_размер)?;
                            проверить_типы(&аргумент.лок(), &параметр.тип, &тип)?;
                            match параметр.тип {
                                Тип::Цел64 | Тип::Лог => {}
                                _ => {
                        ...
                        вызвать_процедуру(имя.лок.clone(), процедура, &mut пп.код, &mut временный_размер)?;
                        if временный_размер > 0 {
                            пп.код.push(Инструкция{ вид: ВидИнструкции::ОсвободитьСоВторогоСтека(временный_размер as u64), лок: имя.лок.clone() });
                        }
                        Ok(())
        ...
        Утверждение::Если{ключ, условие, тело, иначе} => {
            let mut временный_размер = 0;
            let тип = скомпилировать_выражение(пп, имена, &процедура.параметры, &условие, &mut временный_размер)?;
            ...
            for утверждение in тело.iter() {
                скомпилировать_утверждение(пп, имена, процедура, утверждение, отложенные_возвраты)?;
            }
        ...
        Утверждение::Вернуть{ключ, выражение} => {
            if let Some(выражение) = выражение {
                if let Some(результат) = &процедура.результат {
                    let mut временный_размер = 0;
                    let тип = скомпилировать_выражение(пп, имена, &процедура.параметры, &выражение, &mut временный_размер)?;
                    ...
                    if тип.примитивный() {
                        записать_примитивный_тип(&ключ.лок, &mut пп.код, тип.размер())?;
                    } else {
                        пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(тип.размер()), лок: ключ.лок.clone() });
                        пп.код.push(Инструкция{ вид: ВидИнструкции::СкопироватьПамять, лок: ключ.лок.clone() });
                    }
                    if временный_размер > 0 {
...
fn скомпилировать_процедуру(пп: &mut ПП, имена: &Имена, процедура: Процедура) -> Результат<СкомпПроцедура> {
    ...
    for параметр in &процедура.параметры {
        ...
        размер_параметров_на_стеке += выравнять_размер_по_слову(скомп_параметр.тип.размер());
    ...
    let результат = if let Some(тип) = процедура.тип_результата {
        ...
        размер_параметров_на_стеке += выравнять_размер_по_слову(результат.тип.размер());
        Some(результат)
    } else {
        None
    };
    match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            for параметр in скомп_процедура.параметры.iter() {
                ...
                if параметр.тип.примитивный() {
                    записать_примитивный_тип(&процедура.имя.лок, &mut пп.код, параметр.тип.размер())?;
                } else {
                    пп.код.push(Инструкция{ вид: ВидИнструкции::ПротолкнутьЦел(параметр.тип.размер()), лок: процедура.имя.лок.clone() });
            ...
            if let Some(результат) = &скомп_процедура.результат {
                ...
                if результат.тип.примитивный() {
                    прочитать_примитивный_тип(&процедура.имя.лок, &mut пп.код, результат.тип.размер())?;
                    ...
                    пп.код.push(Инструкция{ вид: ВидИнструкции::АргументСоСтека, лок: процедура.имя.лок.clone() });
                    пп.код.push(Инструкция{ вид: ВидИнструкции::ВосстановитьКадрВторогоСтека, лок: процедура.имя.лок.clone() });
                    пп.код.push(Инструкция{ вид: ВидИнструкции::ОсвободитьСоВторогоСтека(размер_параметров_на_стеке as u64 - результат.тип.размер() as u64), лок: процедура.имя.лок.clone() });
                }
            } else {
                пп.код.push(Инструкция{ вид: ВидИнструкции::АргументСоСтека, лок: процедура.имя.лок.clone() });
                пп.код.push(Инструкция{ вид: ВидИнструкции::ВосстановитьКадрВторогоСтека, лок: процедура.имя.лок.clone() });
