...
fn скомпилировать_утверждение(пп: &mut ПП, имена: &Имена, текущая_процедура: &СкомпПроцедура, локальные_имена: &mut ЛокальныеИмена, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::ДекларацияКонстанты{имя, значение, ..} => {
            let (байты_значения, тип) = интерпретировать_выражение_константы(пп, имена, Some(локальные_имена), значение)?;
...
fn интерпретировать_выражение_константы(пп: &ПП, имена: &Имена, локальные_имена: Option<&ЛокальныеИмена>, выражение: &Выражение) -> Результат<(Vec<u8>, Тип)> {
    let mut временный_размер = 0;
    let mut пп = (*пп).clone();
    пп.код.clear();
        ...
        Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
            Выражение::Идент(имя) => match имя.текст.as_str() {
                "массив" => {
                    ...
                    let (байты_размера, тип_размера) = интерпретировать_выражение_константы(&пп, имена, локальные_имена, &выражение_размера)?;
        ...
        loop {
            ...
            match ключ.вид {
                ...
                ВидЛексемы::КлючКонст => {
                    ...
                    let (байты_значения, тип) = интерпретировать_выражение_константы(&self.пп, &self.имена, None, &константа.выражение)?;
                    if тип.примитивный() {
