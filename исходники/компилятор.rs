...
fn скомпилировать_выражение(пп: &mut ПП, имена: &Имена, выражение: &Выражение) -> Результат<Тип> {
    match выражение {
        ...
        Выражение::Строка(строка) => {
            let указатель = пп.данные.len();
            let длинна = строка.текст.len();
            пп.данные.extend(строка.текст.as_bytes());
        ...
        loop {
            ...
            match ключ.вид {
                ВидЛексемы::КлючПер => {
                    ...
                    let адрес = self.пп.данные.len();
                    let новый_размер = self.пп.данные.len() + синтаксис.тип.размер();
                    self.пп.данные.resize(новый_размер, 0u8);
