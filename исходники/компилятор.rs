                    ...
                    if let Выражение::Идент(имя) = массив {
                        ...
                        if let Some(переменная) = имена.переменные.get(&имя.текст) {
                            if let Тип::Массив{размер: _, тип_элемента} = &переменная.тип {
                                ...
                                сделать!(&имя.лок, "Срез должен возвращать временный объект");
                                return Err(());
                                // return Ok(Тип::Срез {
...
fn скомпилировать_утвержление(пп: &mut ПП, имена: &Имена, процедура: &СкомпПроцедура, утверждение: &Утверждение, отложенные_возвраты: &mut Vec<usize>) -> Результат<()> {
    match утверждение {
        ...
        Утверждение::Если{ключ, условие, тело, иначе} => {
            ...
            for утверждение in тело.iter() {
                скомпилировать_утвержление(пп, имена, процедура, утверждение, отложенные_возвраты)?;
            }
    ...
    match процедура.тело {
        ТелоПроцедуры::Внутренее{блок} => {
            ...
            for утверждение in &блок {
                скомпилировать_утвержление(пп, имена, &скомп_процедура, утверждение, &mut отложенные_возвраты)?;
            }
