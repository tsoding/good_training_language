...
fn интерпретировать_выражение_константы(имена: &Имена, локальные_имена: Option<&ЛокальныеИмена>, выражение: &Выражение) -> Результат<i64> {
    match выражение {
        Выражение::НатЧисло(_ключ, число) => Ok(*число as i64),
        Выражение::ЦелЧисло(ключ, число) => if *число >= 0 {
            Ok(*число)
        } else {
        ...
        Выражение::Вызов{вызываемое, аргументы} => match &**вызываемое {
            Выражение::Идент(имя) => match имя.текст.as_str() {
                "массив" => {
                    ...
                    let размер = интерпретировать_выражение_константы(имена, локальные_имена, &выражение_размера)?;
                    let тип_элемента = Box::new(скомпилировать_тип(&тип_элемента, имена, локальные_имена)?);
                    if размер < 0 {
                        диагностика!(выражение_размера.лок(), "ОШИБКА", "Размер массива не может быть отрицательным");
                        return Err(());
                    }
                    Ok(Тип::Массив{размер: размер as usize, тип_элемента})
                }
                "срез" => {
