    ...
    loop {
        ...
        if режим_отладки {
            ...
            for (имя, переменная) in имена.переменные.iter() {
                eprintln!("  {имя}: {адрес:#X} = {:?}", &память[переменная.адрес..переменная.адрес+переменная.тип.размер()], адрес = переменная.адрес);
            }
            loop {
        ...
        match &инструкция.вид {
            ...
            ВидИнструкции::Прочитать64 => {
                ...
                let указатель = стек.pop().unwrap();
                проверить_адрес(&память, указатель, индекс_инструкции, &инструкция)?;
                проверить_адрес(&память, указатель+Тип::Цел64.размер() - 1, индекс_инструкции, &инструкция)?;
                стек.push(usize::from_le_bytes(память[указатель..указатель+Тип::Цел64.размер()].try_into().unwrap()));
                индекс_инструкции += 1;
            }
