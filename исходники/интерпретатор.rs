        ...
        loop {
            ...
            if режим_отладки {
                ...
                if цель_перешагивания.is_none() {
                    ...
                    eprintln!("кадр = {кадр}", кадр = self.кадр_);
                    eprintln!("переменные");
            ...
            match &инструкция.вид {
                ...
                ВидИнструкции::СохранитьКадр => {
                    self.протолкнуть_значение(self.кадр_)?;
                    self.кадр_ = self.стек.len();
                    self.индекс_инструкции += 1;
                }
                ВидИнструкции::ВосстановитьКадр => {
                    self.проверить_арность_аргументов(1)?;
                    self.кадр_ = self.вытолкнуть_значение()?;
                    self.индекс_инструкции += 1;
                }
                &ВидИнструкции::ПрочитатьКадр(смещение) => {
                    self.протолкнуть_значение(self.стек[(self.кадр_ as i32 + смещение) as usize])?;
                    self.индекс_инструкции += 1;
                }
                &ВидИнструкции::ЗаписатьКадр(смещение) => {
                    self.проверить_арность_аргументов(1)?;
                    let значение = self.вытолкнуть_значение()?;
                    self.стек[(self.кадр_ as i32 + смещение) as usize] = значение;
                    self.индекс_инструкции += 1;
                }
                ВидИнструкции::ВыделитьНаВторомСтеке(_размер) => {
                    сделать!(&инструкция.лок, "Интерпретация инструкции {вид_инструкции:?}", вид_инструкции = инструкция.вид);
                    return Err(());
                }
