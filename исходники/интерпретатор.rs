...
macro_rules! ошибка_времени_исполнения {
    ($машина:expr, $($аргы:tt)*) => {{
        ...
        if let Some(инструкция) = $машина.инструкции.get(индекс_инструкции) {
            ...
            eprint!("{путь_к_файлу}:{строка}:{столбец}: {вид_инструкции:?}: ");
        }
    ...
    loop {
        ...
        match &инструкция.вид {
            ...
            &ВидИнструкции::ПрочитатьКадр(смещение) => {
                let адрес = машина.кадр - (смещение + 1)*РАЗМЕР_СЛОВА;
                let значение = u64::from_le_bytes(машина.срез_памяти(адрес, РАЗМЕР_СЛОВА)?.try_into().unwrap()) as usize;
                машина.протолкнуть_значение(значение)?;
                машина.индекс_инструкции += 1;
            }
            &ВидИнструкции::ЗаписатьКадр(смещение) => {
                ...
                let адрес = машина.кадр - (смещение + 1)*РАЗМЕР_СЛОВА;
                машина.срез_памяти(адрес, РАЗМЕР_СЛОВА)?.copy_from_slice(&значение.to_le_bytes());
                машина.индекс_инструкции += 1;
            }
