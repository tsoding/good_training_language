...
fn проверить_адрес(данные: &Vec<u8>, указатель: usize, индекс_инструкции: usize, инструкция: &Инструкция) -> Результат<()> {
    if указатель >= данные.len() {
        eprintln!("{индекс_инструкции}: {инструкция:?}: ОШИБКА ВРЕМЕНИ ИСПОЛНЕНИЯ: инструкция попыталось получить доступ к некорректному адресу данных {указатель}. Максимально возможный {макс}.", макс = данные.len());
        return Err(())
    } else {
        Ok(())
    }
...
pub fn интерпретировать(программа: &Программа, процедура_точки_входа: &str) -> Результат<()> {
    let mut индекс_инструкции = match программа.точки_входа_процедур.get(процедура_точки_входа) {
