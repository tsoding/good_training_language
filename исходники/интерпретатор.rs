...
pub struct Машина<'ы> {
    ...
    pub стек: Вектор<usize>,
    начало_данных: usize,
    начало_второго_стека: usize,
    pub память: Вектор<u8>,
...
macro_rules! ошибка_времени_исполнения {
    ($машина:expr, $($аргы:tt)*) => {{
        ...
        if let Некий(инструкция) = $машина.инструкции.get(индекс_инструкции) {
...
impl<'ы> Машина<'ы> {
    ...
    fn вытолкнуть_значение_нат(&mut self) -> Результат<usize> {
        if let Некий(значение) = self.стек.pop()  {
            Хорош(значение)
        } else {
    ...
    fn вытолкнуть_значение_вещ32(&mut self) -> Результат<f32> {
        Хорош(f32::from_le_bytes(self.вытолкнуть_значение_нат()?.to_le_bytes()[0..4].try_into().unwrap()))
    }
    ...
    pub fn срез_памяти(&mut self, адрес: usize, размер: usize) -> Результат<&mut [u8]> {
        ...
        if let Некий(срез) = self.память.get_mut(адрес..адрес+размер) {
            Хорош(срез)
        } else {
            ошибка_времени_исполнения!(self, "Попытка получить доступ к некорректнному диапазону памяти [{начало}..{конец}). Разрешенный диапазон [{мин}..{макс})", начало = адрес, конец = адрес+размер);
            Ошиб(())
    ...
    fn инструкция(&self) -> Результат<&Инструкция> {
        match self.инструкции.get(self.индекс_инструкции) {
            Некий(инструкция) => Хорош(инструкция),
            Нету => {
    ...
    pub fn интерпретировать(&mut self, имена: &Имена, точка_входа: usize, режим_отладки: bool) -> Результат<()> {
        ...
        let mut цель_перешагивания: Опция<usize> = Нету;
        ...
        loop {
            ...
            if режим_отладки {
                if let Некий(цель) = цель_перешагивания.clone() {
                    if глубина_вызовов <= цель {
                        цель_перешагивания = Нету;
                    }
                }

                if цель_перешагивания.это_нету() {
                        ...
                        match аргы.as_slice() {
                            ...
                            ["инст", парам @ ..] => match парам {
                                [инст] => match инст.parse::<usize>() {
                                    Хорош(индекс_инструкции) => if let Некий(инструкция) = self.инструкции.get(индекс_инструкции) {
                                        диагностика!(&инструкция.лок, "ИНСТРУКЦИЯ", "{индекс_инструкции}: {вид_инструкции:?}", вид_инструкции = инструкция.вид);
                                    } else {
                            ...
                            ["перешаг", ..] => {
                                цель_перешагивания = Некий(глубина_вызовов);
                                break
                            }
                            [команда, ..] => {
