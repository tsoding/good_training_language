...
impl ВидБинопа {
    const МАКС_ПРИОРИТЕТ: usize = 5;

    fn приоритет(&self) -> usize {
        use self::ВидБинопа::*;
        match self {
            Или                           => Self::МАКС_ПРИОРИТЕТ - 5,
            Меньше | Больше | Равно       => Self::МАКС_ПРИОРИТЕТ - 4,
            Сложение | Вычитание          => Self::МАКС_ПРИОРИТЕТ - 3,
            Умножение | Деление | Остаток => Self::МАКС_ПРИОРИТЕТ - 2,
...
pub enum Утверждение {
    ...
    Декларация{ключ: Лексема, имя: Лексема, тип: Выражение, значение: Option<Выражение>},
}
...
fn разобрать_утверждение(лекс: &mut Лексер) -> Результат<Утверждение> {
    match лекс.подсмотреть_лексему()?.вид {
        ...
        ВидЛексемы::КлючПер => {
            ...
            Ok(Утверждение::Декларация{ключ, имя, тип, значение})
        }
        _ => {
