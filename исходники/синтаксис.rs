...
impl ВидБинопа {
    ...
    fn по_виду_лексемы(вид: &ВидЛексемы) -> Option<ВидБинопа> {
        match вид {
            ВидЛексемы::Равно           => Some(ВидБинопа::Равно),
            ВидЛексемы::Меньше          => Some(ВидБинопа::Меньше),
            ВидЛексемы::Больше          => Some(ВидБинопа::Больше),
            ВидЛексемы::МеньшеРавно     => Some(ВидБинопа::МеньшеРавно),
            ВидЛексемы::БольшеРавно     => Some(ВидБинопа::БольшеРавно),
            ВидЛексемы::НеРавно         => Some(ВидБинопа::НеРавно),
            ...
            ВидЛексемы::КлючЛибо        => Some(ВидБинопа::Либо),
...
pub enum Выражение {
    ...
    Строка(Лексема),
    Идент(Лексема),
    ЦепочкаВызовов{имя: Лексема, цепочка_аргументов: Vec<Аргументы>},
    УнарныйМинус {
        ключ: Лексема,
...
impl Выражение {
    ...
    fn разобрать_первичное(лекс: &mut Лексер) -> Результат<Выражение> {
        ...
        match лексема.вид {
            ВидЛексемы::ЦелШестЧисло => {
                ...
                if лекс.подсмотреть_лексему()?.вид == ВидЛексемы::Идент {
                    if лекс.подсмотреть_лексему()?.текст == "нат" {
                        ...
                        return Ok(Выражение::НатЧисло(лексема, число as u64));
                    }
                }
            ...
            ВидЛексемы::Идент => {
                let mut цепочка_аргументов = Vec::new();
                ...
                if цепочка_аргументов.len() == 0 {
                    Ok(Выражение::Идент(лексема))
                } else {
                    Ok(Выражение::ЦепочкаВызовов {
                        имя: лексема,
                        цепочка_аргументов,
                    })
                }
...
pub enum Утверждение {
    ...
    Вилка {ключ: Лексема, выражение: Выражение, ветки: Vec<ВеткаВилки>, любое: Option<(Лексема, Vec<Утверждение>)>},
}
...
fn разобрать_утверждение(лекс: &mut Лексер) -> Результат<Утверждение> {
    match лекс.подсмотреть_лексему()?.вид {
        ...
        ВидЛексемы::КлючВилка => {
                ...
                match ключ_ветки.вид {
                    ВидЛексемы::КлючКогда => {
                        if лекс.подсмотреть_лексему()?.вид == ВидЛексемы::КлючЛюбое {
                            let _ = лекс.вытащить_лексему().unwrap();
                            любое = Some((ключ_ветки, разобрать_блок_кода(лекс)?));
            ...
            Ok(Утверждение::Вилка{ключ, выражение, ветки, любое})
        }
        _ => {
