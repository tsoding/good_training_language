...
impl ВидБинопа {
    const МАКС_ПРИОРИТЕТ: usize = 4;

    fn приоритет(&self) -> usize {
        use self::ВидБинопа::*;
        match self {
            Или                           => Self::МАКС_ПРИОРИТЕТ - 4,
            Меньше | Больше | Равно       => Self::МАКС_ПРИОРИТЕТ - 3,
            Сложение | Вычитание          => Self::МАКС_ПРИОРИТЕТ - 2,
            Умножение | Деление | Остаток => Self::МАКС_ПРИОРИТЕТ - 1,
            Как                           => Self::МАКС_ПРИОРИТЕТ,
        }
    }
    ...
    match лексема.вид {
        ВидЛексемы::Идент => {
            ...
            match лексема.вид {
                ...
                ВидЛексемы::ОткрытаяСкобка => {
                    let mut аргументы = разобрать_список_аргументов_вызова(лекс)?;
                    let лексема = лекс.вытащить_лексему_вида(&[ВидЛексемы::ТочкаЗапятая, ВидЛексемы::Присваивание])?;
                    match лексема.вид {
                        ВидЛексемы::ТочкаЗапятая => Ok(Утверждение::Вызов {имя, аргументы}),
                        ВидЛексемы::Присваивание => {
                            let ключ = лексема;
                            if аргументы.len() != 1 {
                            ...
                            let индекс = аргументы.pop().unwrap();
                            let значение = Выражение::разобрать(лекс)?;
                            let _ = лекс.вытащить_лексему_вида(&[ВидЛексемы::ТочкаЗапятая])?;
