...
const КОМАНДЫ: &[Команда] = &[
    Команда {
        имя: "комп",
        сигнатура: "[-пуск] [-цель <цель>] [-вывод <файл-вывода>] <файл-ввода>",
        ...
        запустить: |программа, mut аргы| {
            ...
            let mut цель = Цель::ЛинуксИкс86_64Фазм;
                ...
                match аргы.next() {
                    Some(арг) => match арг.as_str() {
                        ...
                        "-цель" => {
                            match аргы.next() {
                                Some(арг) => match арг.as_str() {
                                    "эльф" => цель = Цель::ЛинуксИкс86_64Эльф,
                                    "фазм" => цель = Цель::ЛинуксИкс86_64Фазм,
                                    _ => {
                                        eprintln!("ОШИБКА: неизвестная цель «{арг}»");
                                        // СДЕЛАТЬ: напечатать список возможных целей
                                        return Err(())
                                    }
                                }
                                None => {
            ...
            if let Some(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                ...
                match цель {
                    Цель::ЛинуксИкс86_64Эльф => {
                        эльф::сгенерировать_исполняемый_файл(&путь_к_исполняемому, &программа.пп, точка_входа)?;
                    }
                    Цель::ЛинуксИкс86_64Фазм => {
    ...
    Команда {
        ...
        запустить: |программа, mut аргы| {
            ...
            if let Some(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                ...
                let объём_стека = 1_000_000;         // 1 мегабайт (как на винде)
                let mut машина = Машина::новая(&программа.пп, объём_стека);
                машина.интерпретировать(&программа.имена.переменные, точка_входа, режим_отладки)
            } else {
                eprintln!("ОШИБКА: процедура точки входа «{процедура_точки_входа}» не найдена! Пожалуйста определите её!");
                Err(())
            }
    ...
    Команда {
        имя: "объект",
        сигнатура: " [-цель <цель>] <путь_к_файлу>",
        описание: "Сгенерировать объектный файл, который можно скомпоновать со статическими библиотеками.",
        запустить: |программа, mut аргы| {
            let mut файл_вывода = None;
            let mut цель = Цель::ЛинуксИкс86_64Фазм;
            ...
            loop {
                match аргы.next() {
                    Some(арг) => match арг.as_str() {
                        ...
                        _ => {
                            if файл_вывода.is_some() {
                                пример(программа);
                                eprintln!("ОШИБКА: Неизвестный флаг «{арг}».");
                                return Err(())
                            } else {
                                файл_вывода = Some(арг)
                            }
                        }
                    }
                    None => break,
            ...
            let файл_вывода = if let Some(файл_вывода) = аргы.next() {
                PathBuf::from(файл_вывода)
            } else {
                пример(программа);
                eprintln!("ОШИБКА: требуется путь к файлу!");
                return Err(());
            };

            match цель {
                Цель::ЛинуксИкс86_64Фазм => фазм::сгенерировать_объектный_файл(&файл_вывода),
                Цель::ЛинуксИкс86_64Эльф => эльф::сгенерировать_объектный_файл(&файл_вывода),
            }
        }
    },
];
