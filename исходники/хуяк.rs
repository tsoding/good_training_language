...
use лексика::Лексер;
use компилятор::{Программа, Инструкция};
...
const КОМАНДЫ: &[Команда] = &[
    Команда {
        ...
        запустить: |путь_к_файлу| {
            ...
            эльф::сгенерировать(&Path::new(&путь_к_файлу).with_extension(""), &программа.пп)
        },
    },
    Команда {
        имя: "интер",
        ...
        запустить: |путь_к_файлу| {
            ...
            if let Some(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                интерпретировать(&программа.пп, процедура.точка_входа)
            } else {
    ...
    Команда {
        ...
        запустить: |путь_к_файлу| {
            ...
            программа.пп.код.push(Инструкция::ПротолкнутьЦелое(1));
            программа.пп.код.push(Инструкция::ЛогОтрицание);
            программа.пп.код.push(Инструкция::Возврат);
            программа.пп.вывалить();
            эльф::сгенерировать(&Path::new(путь_к_файлу).with_extension(""), &программа.пп)
        }
    }
