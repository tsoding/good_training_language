...
type Результат<Тэ> = стд::Результат<Тэ, ()>;

fn прочитать_содержимое_файла(путь_к_файлу: &Path, лок: Опция<Лок>) -> Результат<Вектор<char>> {
        ...
        .map_err(|ошибка| {
            ...
            match ошибка.kind() {
                io::ErrorKind::NotFound => match лок {
                    Некий(лок) => диагностика!(лок, "ОШИБКА", "файл «{путь_к_файлу}» не найден", путь_к_файлу = путь_к_файлу.display()),
                    Нету => eprintln!("ОШИБКА: файл «{путь_к_файлу}» не найден", путь_к_файлу = путь_к_файлу.display()),
                }
                _ => match лок {
                    Некий(лок) => диагностика!(лок, "ОШИБКА", "не получилось прочитать файл «{путь_к_файлу}»: {ошибка}", путь_к_файлу = путь_к_файлу.display()),
                    Нету => eprintln!("ОШИБКА: не получилось прочитать файл «{путь_к_файлу}»: {ошибка}", путь_к_файлу = путь_к_файлу.display()),
                }
            }
        })
}
...
const КОМАНДЫ: &[Команда] = &[
    Команда {
        ...
        запустить: |программа, mut аргы| {
            let mut пуск = false;
            let mut файл_ввода = Нету;
            let mut файл_вывода = Нету;

            loop {
                match аргы.next() {
                    Некий(арг) => match арг.as_str() {
                        "-пуск" => пуск = true,
                        "-вывод" => {
                            match аргы.next() {
                                Некий(арг) => файл_вывода = Некий(арг),
                                Нету => {
                        ...
                        _ => {
                            if файл_ввода.это_некий() {
                                пример(программа);
                                eprintln!("ОШИБКА: Неизвестный флаг «{арг}».");
                                return Ошиб(())
                            } else {
                                файл_ввода = Некий(арг)
                            }
                        }
                    }
                    Нету => break,
            ...
            let файл_ввода = if let Некий(файл_ввода) = файл_ввода {
            ...
            let содержимое: Вектор<char> = прочитать_содержимое_файла(&файл_ввода, Нету)?;
            ...
            if let Некий(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                ...
                if пуск {
                    ...
                    #[cfg(all(unix))] {
                        ...
                        if let Некий(сигнал) = код_выхода.signal() {
    ...
    Команда {
        ...
        запустить: |программа, mut аргы| {
            ...
            loop {
                match аргы.next() {
                    Некий(арг) => match арг.as_str() {
                        ...
                        _ => {
                            if путь_к_файлу.это_некий() {
                                пример(программа);
                                eprintln!("ОШИБКА: неизвестный флаг «{арг}»");
                                return Ошиб(())
                            } else {
                                путь_к_файлу = Некий(арг)
                            }
                        }
                    }
                    Нету => break,
            ...
            let путь_к_файлу = if let Некий(путь_к_файлу) = путь_к_файлу {
            ...
            let содержимое: Вектор<char> = прочитать_содержимое_файла(&путь_к_файлу, Нету)?;
    ...
    Команда {
        ...
        запустить: |программа, mut аргы| {
            let путь_к_файлу = if let Некий(путь_к_файлу) = аргы.next() {
                PathBuf::from(путь_к_файлу)
            } else {
                пример(программа);
                eprintln!("ОШИБКА: требуется файл с программой!");
    ...
    Команда {
        ...
        запустить: |программа, mut аргы| {
            if let Некий(_имя_команды) = аргы.next() {
...
fn главная() -> Результат<()> {
    ...
    let имя_команды = if let Некий(имя_команды) = аргы.next() {
        имя_команды
    } else {
        пример(&программа);
    ...
    if let Некий(команда) = КОМАНДЫ.iter().find(|команда| имя_команды == команда.имя) {
...
fn main() -> ExitCode {
    match главная() {
        Хорош(()) => ExitCode::SUCCESS,
        Ошиб(()) => ExitCode::FAILURE,
    }
}
