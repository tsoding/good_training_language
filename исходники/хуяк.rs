...
use лексика::Лексер;
use компилятор::{Программа, Инструкция, ПП};
...
const КОМАНДЫ: &[Команда] = &[
    Команда {
        ...
        запустить: |путь_к_файлу| {
            ...
            if let Some(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                эльф::сгенерировать(&Path::new(&путь_к_файлу).with_extension(""), &программа.пп, процедура.точка_входа)
            } else {
    ...
    Команда {
        ...
        запустить: |путь_к_файлу| {
            ...
            пп.код.push(Инструкция::ПротолкнутьЦелое(1));
