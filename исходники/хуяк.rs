...
fn главная() -> Результат<()> {
    todo!("СДЕЛАТЬ: Чтобы сохранить правильный порядок компиляции процедур и переменных, требуется компилировать по мере того как разбираем. Т.е. нужно слить вместе модули «пп» и «синтаксис»");
    return Err(());
