...
enum Утверждение {
    Присваивание{ переменная: Лексема, значение: Выражение }
}
...
fn разобрать_выражение(лекс: &mut Лексер) -> Option<Выражение> {
    let лексема = лекс.ожидать_виды(&[ВидЛексемы::Число])?;
...
fn разобрать_процедуру(лекс: &mut Лексер) -> Option<Процедура> {
    ...
    loop {
        ...
        match лексема.вид {
            ВидЛексемы::Идент => {
                let переменная = лексема;
                let _ = лекс.ожидать_виды(&[ВидЛексемы::Присваивание])?;
